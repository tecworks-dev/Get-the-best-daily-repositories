"""
Observability for Hurley Agentic.
"""
import os
import json
...
def setup_observer(config: AgentConfig) -> bool:
    ...
    if config.observer == ObserverType.ARIZE_PHOENIX:
        ...
        if not phoenix_endpoint:
            px.launch_app()
            tracer_provider = register(endpoint='http://localhost:6006/v1/traces', project_name="hurley-agentic")
            ...
            tracer_provider = register(endpoint=phoenix_endpoint, project_name="hurley-agentic")
...
def eval_fcs() -> None:
    '''
    Evaluate the FCS score for the HurleyQueryEngine._query span.
    '''
    import phoenix as px
    ...
    client = px.Client()
    all_spans = client.query_spans(query, project_name="hurley-agentic")
    hurley_spans = all_spans[all_spans['name'] == 'HurleyQueryEngine._query'].copy()
    hurley_spans['top_level_parent_id'] = hurley_spans.apply(
        lambda row: _find_top_level_parent_id(row, all_spans), axis=1
    )
    hurley_spans['score'] = hurley_spans['output.value'].apply(_extract_fcs_value)

    hurley_spans.reset_index(inplace=True)
    top_level_spans = hurley_spans.copy()
    top_level_spans['context.span_id'] = top_level_spans['top_level_parent_id']
    hurley_spans = pd.concat([hurley_spans, top_level_spans], ignore_index=True)
    hurley_spans.set_index('context.span_id', inplace=True)

    px.Client().log_evaluations(
        SpanEvaluations(
            dataframe=hurley_spans,
            eval_name="Hurley FCS",
        ),
    )
