...
from .tools import HurleyToolFactory, HurleyTool, ToolsFactory
...
class Agent:
    ...
    def from_corpus(
        ...
        query_logging_callback: Optional[Callable[[str, str], None]] = None,
        verbose: bool = False,
        hurley_filter_fields: list[dict] = [],
        hurley_lambda_val: float = 0.005,
        hurley_reranker: str = "mmr",
        hurley_rerank_k: int = 50,
        hurley_n_sentences_before: int = 2,
        hurley_n_sentences_after: int = 2,
        hurley_summary_num_results: int = 10,
        hurley_summarizer: str = "hurley-summary-ext-24-05-sml",
    ) -> "Agent":
        """
        Create an agent from a single Hurley corpus

        Args:
            tool_name (str): The name of Hurley tool used by the agent
            ...
            hurley_filter_fields (List[dict], optional): The filterable attributes
                (each dict maps field name to Tuple[type, description]).
            hurley_lambda_val (float, optional): The lambda value for Hurley hybrid search.
            hurley_reranker (str, optional): The Hurley reranker name (default "mmr")
            hurley_rerank_k (int, optional): The number of results to use with reranking.
            hurley_n_sentences_before (int, optional): The number of sentences before the matching text
            hurley_n_sentences_after (int, optional): The number of sentences after the matching text.
            hurley_summary_num_results (int, optional): The number of results to use in summarization.
            hurley_summarizer (str, optional): The Hurley summarizer name.
        ...
        for field in hurley_filter_fields:
        ...
        hurley_tool = vec_factory.create_rag_tool(
            tool_name=tool_name or f"hurley_{hurley_corpus_id}",
            tool_description=f"""
            Given a user query,
            returns a response (str) to a user question about {data_description}.
            """,
            tool_args_schema=query_args,
            reranker=hurley_reranker,
            rerank_k=hurley_rerank_k,
            n_sentences_before=hurley_n_sentences_before,
            n_sentences_after=hurley_n_sentences_after,
            lambda_val=hurley_lambda_val,
            summary_num_results=hurley_summary_num_results,
            hurley_summarizer=hurley_summarizer,
    ...
    async def achat(self, prompt: str) -> AgentResponse:    # type: ignore
        ...
        try:
            ...
            return AgentResponse(
                response = (
                    f"Hurley Agentic: encountered an exception ({e}) at ({traceback.format_exc()})"
                    ", and can't respond."
                )
            )
    ...
    async def astream_chat(self, prompt: str) -> AgentStreamingResponse:    # type: ignore
        ...
        try:
            ...
            raise ValueError(
                f"Hurley Agentic: encountered an exception ({e}) at ({traceback.format_exc()}), and can't respond."
            ) from e
