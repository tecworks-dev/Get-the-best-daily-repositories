...
from llama_index.indices.managed.hurley import HurleyIndex
...
class HurleyToolMetadata(ToolMetadata):
    """
    ...
    def __repr__(self) -> str:
        """
        Returns a string representation of the HurleyToolMetadata object, including the tool_type attribute.
        """
        base_repr = super().__repr__()
        return f"{base_repr}, tool_type={self.tool_type}"

class HurleyTool(FunctionTool):
    """
    ...
    def __init__(
        ...
        vm = HurleyToolMetadata(tool_type=tool_type, **metadata_dict)
    ...
    def from_defaults(
        ...
        hurley_tool = cls(tool_type=tool_type, fn=tool.fn, metadata=tool.metadata, async_fn=tool.async_fn)
        return hurley_tool
...
def _build_filter_string(kwargs: Dict[str, Any], tool_args_type: Dict[str, str], fixed_filter: str) -> str:
    """
    Build filter string for Hurley from kwargs
    """
    filter_parts = []
...
class HurleyToolFactory:
    """
    A factory class for creating Hurley RAG tools.
    """

    def __init__(
        self,
        hurley_customer_id: str = str(os.environ.get("VECTARA_CUSTOMER_ID", "")),
        hurley_corpus_id: str = str(os.environ.get("VECTARA_CORPUS_ID", "")),
        hurley_api_key: str = str(os.environ.get("VECTARA_API_KEY", "")),
    ) -> None:
        """
        Initialize the HurleyToolFactory
        Args:
            hurley_customer_id (str): The Hurley customer ID.
            hurley_corpus_id (str): The Hurley corpus ID (or comma separated list of IDs).
            hurley_api_key (str): The Hurley API key.
        """
        self.hurley_customer_id = hurley_customer_id
        self.hurley_corpus_id = hurley_corpus_id
        self.hurley_api_key = hurley_api_key
        self.num_corpora = len(hurley_corpus_id.split(","))
    ...
    def create_search_tool(
        ...
        """
        Creates a Hurley search/retrieval tool
        ...
        Args:
            ...
            fixed_filter (str, optional): A fixed Hurley filter condition to apply to all queries.
            lambda_val (float, optional): Lambda value for the Hurley query.
        ...
        Returns:
            HurleyTool: A HurleyTool object.
        """

        hurley = HurleyIndex(
            hurley_api_key=self.hurley_api_key,
            hurley_customer_id=self.hurley_customer_id,
            hurley_corpus_id=self.hurley_corpus_id,
            x_source_str="hurley-agentic",
        )

        # Dynamically generate the search function
        def search_function(*args, **kwargs) -> ToolOutput:
            """
            Dynamically generated function for semantic search Hurley.
            """
            ...
            hurley_retriever = hurley.as_retriever(
            ...
            response = hurley_retriever.retrieve(query)

            if len(response) == 0:
                msg = "Hurley Tool failed to retreive any results for the query."
                return ToolOutput(
                    tool_name=search_function.__name__,
                    content=msg,
                    raw_input={"args": args, "kwargs": kwargs},
    ...
    def create_rag_tool(
        ...
        tool_args_type: Dict[str, str] = {},
        fixed_filter: str = "",
        hurley_summarizer: str = "hurley-summary-ext-24-05-sml",
        hurley_prompt_text: str = None,
        ...
        Args:
            ...
            tool_args_type (Dict[str, str], optional): The type of each argument (doc or part).
            fixed_filter (str, optional): A fixed Hurley filter condition to apply to all queries.
            hurley_summarizer (str, optional): The Hurley summarizer to use.
            hurley_prompt_text (str, optional): The prompt text for the Hurley summarizer.
            ...
            include_citations (bool, optional): Whether to include citations in the response.
                If True, uses markdown hurley citations that requires the Hurley scale plan.
        ...
        def rag_function(*args, **kwargs) -> ToolOutput:
            """
            Dynamically generated function for RAG query with Hurley.
            """
            # Convert args to kwargs using the function signature
            ...
            hurley_query_engine = hurley.as_query_engine(
                summary_enabled=True,
                summary_num_results=summary_num_results,
                summary_response_lang=summary_response_lang,
                summary_prompt_name=hurley_summarizer,
                prompt_text=hurley_prompt_text,
                reranker=reranker,
                rerank_k=rerank_k if rerank_k * self.num_corpora <= 100 else int(100 / self.num_corpora),
                mmr_diversity_bias=mmr_diversity_bias,
                udf_expression=udf_expression,
            ...
            response = hurley_query_engine.query(query)
...
class ToolsFactory:
    ...
    def create_tool(self, function: Callable, tool_type: ToolType = ToolType.QUERY) -> HurleyTool:
        """
        ...
        Returns:
            HurleyTool: A HurleyTool object.
        """
        return HurleyTool.from_defaults(tool_type=tool_type, fn=function)
    ...
    def get_llama_index_tools(
        ...
        **kwargs: dict,
        ...
        Returns:
            List[HurleyTool]: A list of HurleyTool objects.
        """
        # Dynamically install and import the module
        if tool_package_name not in LI_packages:
            raise ValueError(f"Tool package {tool_package_name} from LlamaIndex not supported by Hurley-agentic.")
        ...
        for tool in tools:
            ...
            if isinstance(func_type, dict):
                ...
                tool_type = func_type
            vtool = HurleyTool(tool_type=tool_type, fn=tool.fn, metadata=tool.metadata, async_fn=tool.async_fn)
            vtools.append(vtool)
        return vtools
