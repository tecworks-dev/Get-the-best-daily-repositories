...
/// ```no_run
/// use nio::fs::File;
/// use nio::io::AsyncWriteExt; // for write_all()
///
...
/// ```no_run
/// use nio::fs::File;
/// use nio::io::AsyncReadExt; // for read_to_end()
///
/// # async fn dox() -> std::io::Result<()> {
...
impl File {
    ...
    /// # Errors
    ///
    /// This function will return an error if called from outside of the Nio
    ...
    /// # Errors
    ///
    /// Results in an error if called from outside of the Nio runtime or if
    ...
    /// Converts a [`std::fs::File`] to a [`nio::fs::File`](File).
    ///
    /// # Examples
    ///
    /// ```no_run
    /// // This line could block. It is not recommended to do this on the Nio
    /// // runtime.
    /// let std_file = std::fs::File::open("foo.txt").unwrap();
    /// let file = nio::fs::File::from_std(std_file);
    /// ```
    ...
    ///
    /// Although Nio uses a sensible default value for this buffer size, this function would be
