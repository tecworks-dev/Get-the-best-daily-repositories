...
use nio::net::TcpStream;
use nio::task::JoinHandle;
use std::pin::pin;
use std::time::Duration;

async fn make_socketpair() -> (TcpStream, TcpStream) {
    let listener = nio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let connector = TcpStream::connect(addr);
    let acceptor = listener.accept();

    let (c1, c2) = join!(connector, acceptor);
...
async fn block_write(s: &mut TcpStream) -> usize {
    ...
    let mut copied = 0;
    let mut sleep = pin!(nio::time::sleep(Duration::from_millis(10)));
    loop {
        select! {
            _ = &mut sleep => {
                break;
    ...
    Fut: std::future::Future<Output = ()>,
{
    ...
    let handle = nio::spawn(async move { copy_bidirectional(&mut a1, &mut b1).await });
    cb(handle, a, b).await;

    let (a, mut a1) = make_socketpair().await;
    let (b, mut b1) = make_socketpair().await;

    let handle = nio::spawn(async move { copy_bidirectional(&mut b1, &mut a1).await });
