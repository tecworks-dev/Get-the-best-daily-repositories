...
pub mod macro_rt_arg_unhandled_panic {
    ...
    #[nio::test(flavor = "current_thread", unhandled_panic = "shutdown_runtime")]
    #[should_panic]
    async fn unhandled_panic_shutdown_runtime() {
        let _ = nio::spawn(async {
            panic!("This panic should shutdown the runtime.");
        })
        .await;
    }

    #[nio::test(flavor = "current_thread", unhandled_panic = "ignore")]
    async fn unhandled_panic_ignore() {
        let rt = nio::spawn(async {
