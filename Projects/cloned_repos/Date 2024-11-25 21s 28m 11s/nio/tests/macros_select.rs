...
async fn biased_eventually_ready() {
    use nio::task::yield_now;

    let mut one = pin!(async {});
    let mut two = pin!(async { yield_now().await });
    let mut three = pin!(async { yield_now().await });
...
mod unstable {
    ...
    fn deterministic_select_multi_thread() {
        let seed = b"bytes used to generate seed";
        let rt1 = nio::runtime::Builder::new_multi_thread()
            .worker_threads(1)
            .rng_seed(RngSeed::from_bytes(seed))
            .build()
            .unwrap();
        let rt1_values = rt1.block_on(async {
            let _ = nio::spawn(async { (select_0_to_9().await, select_0_to_9().await) }).await;
        });

        let rt2 = nio::runtime::Builder::new_multi_thread()
            .worker_threads(1)
            .rng_seed(RngSeed::from_bytes(seed))
            .build()
            .unwrap();
