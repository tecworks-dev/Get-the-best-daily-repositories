...
async fn sync_two_lit_expr_comma() {
    let foo = try_join!(async { ok(1) }, async { ok(2) },);
...
async fn two_await() {
    ...
    let mut join = task::spawn(async { try_join!(rx1, rx2) });
...
async fn err_abort_early() {
    ...
    let mut join = task::spawn(async { try_join!(rx1, rx2, rx3) });
...
async fn empty_try_join() {
    assert_eq!(try_join!() as Result<_, ()>, Ok(()));
}
