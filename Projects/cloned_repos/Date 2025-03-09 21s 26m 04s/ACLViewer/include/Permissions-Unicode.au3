...
;Global Const $FILE_USERS_DEFAULT		 = BitOR($GENERIC_READ,$FILE_LIST_DIRECTORY,$FILE_READ_DATA,$GENERIC_EXECUTE) ;Read & execute.
Global Const $FILE_AUTH_USERS_DEFAULT	 = BitOR($GENERIC_READ,$GENERIC_EXECUTE,$GENERIC_WRITE,$DELETE) ;Modify.
...
; Syntax.........: _SetDefaultFileAccess($Path, $Recurse = 0)
...
; Description ...: Grants everyone read access, and denies everyone write access.
...
; Syntax.........: _SetObjectPermissions($oName, $aPermissions, $_SE_OBJECT_TYPE, $SetOwner, $ClearDacl, $Recurse)
...
;                  Failure      - 0 and sets @error
; Author ........: FredAI
; Modified.......: AdamUL - Added Unicode support.
; Remarks .......: Unlike _SetObjectPermissions, this function is able to edit the inherithed entries in the DACL, instead of adding new ones.
;					+ For instance if an object has a denied ace entry this function can edit it and make it a granted acces ace.
; Related .......: _SetObjectPermissions, _GrantReadAccess, _GrantAllAccess, _DenyAllAccess, _GrantReadDenyWrite
; Link ..........:
...
Func _MergeDaclToArray(ByRef $Dacl, ByRef $aPerm, $Filter = 1)
    ...
    Local $aCall = DllCall($h__Advapi32Dll,'DWORD','GetExplicitEntriesFromAclW','ptr',$Dacl,'ulong*',0,'ptr*',0)
    If @error Or $aCall[0] Then Return SetError(2,0,0)
    Local $uB = UBound($aPerm), $l = 0, $TrusteeExists, $E = $aCall[2], $eaSID, $pEa = $aCall[3]
...
Func _CreateDaclFromArray(ByRef $aPermissions, ByRef $InHerit)
	...
	For $i = 0 To $uB -1
		...
		$aCall = DllCall($h__Advapi32Dll,'BOOL','LookupAccountSidW','ptr',0,'ptr',$pSID[$i],'ptr*',0,'dword*',32,'ptr*',0,'dword*',32,'dword*',0)
	...
	$aCall = DllCall($h__Advapi32Dll,'DWORD','SetEntriesInAclW','ULONG',$uB,'ptr',$p_EXPLICIT_ACCESS ,'ptr',0,'ptr*',0)
...
Func _SetObjectSecurity($oName, $_SE_OBJECT_TYPE, $SECURITY_INFORMATION, $pOwner = 0, $pGroup = 0, $Dacl = 0, $Sacl = 0)
	...
	Else ;If it's a named object
		...
		$aCall = DllCall($h__Advapi32Dll,'dword','SetNamedSecurityInfoW','wstr',$oName,'dword',$_SE_OBJECT_TYPE, _
		'dword',$SECURITY_INFORMATION,'ptr',$pOwner,'ptr',$pGroup,'ptr',$Dacl,'ptr',$Sacl)
	EndIf
...
Func _ClearObjectDacl($oName, $_SE_OBJECT_TYPE = $SE_FILE_OBJECT)
	...
	Else
		If $_SE_OBJECT_TYPE = $SE_REGISTRY_KEY Then $oName = _Security_RegKeyName($oName)
		DllCall($h__Advapi32Dll,'DWORD','SetNamedSecurityInfoW','wstr',$oName,'dword',$_SE_OBJECT_TYPE,'DWORD',4,'ptr',0,'ptr',0,'ptr',0,'ptr',0)
		$aRet = DllCall($h__Advapi32Dll,'DWORD','SetNamedSecurityInfoW','wstr',$oName,'dword',$_SE_OBJECT_TYPE,'DWORD',4,'ptr',0,'ptr',0,'ptr',$DACL,'ptr',0)
	EndIf
	If @error Then Return SetError(@error,0,0)
...
; #FUNCTION# ====================================================================================================================
; Name...........: _GetObjectDACLIncludeInherit
; Description ...: Returns a pointer to the the DACL structure of an object including inherited ACEs.
; Syntax.........: _GetObjectDACLIncludeInherit($oName, $_SE_OBJECT_TYPE)
; Parameters ....:  $oName   - The name or handle to the object. This can be a path to a file or folder, a registry key,
;					+ a service name, a process handle, etc. See the comments on the _SE_OBJECT_TYPE enum for more info.
;					$_SE_OBJECT_TYPE - The type of the object to set permissions. This must be one of the values of the
;						+_SE_OBJECT_TYPE enum. The default is $SE_FILE_OBJECT (a file or folder).
...
Func _SetObjectOwner($oName, $_SE_OBJECT_TYPE = $SE_FILE_OBJECT, $AccountName = 'Administrators')
	...
	Else
		If $_SE_OBJECT_TYPE = $SE_REGISTRY_KEY Then $oName = _Security_RegKeyName($oName)
		$aRet = DllCall($h__Advapi32Dll,'DWORD','SetNamedSecurityInfoW','wstr',$oName,'dword',$_SE_OBJECT_TYPE,'DWORD',1,'ptr',$pSID,'ptr',0,'ptr',0,'ptr',0)
	EndIf
...
Func _GetObjectSecurityDescriptor($oName, $_SE_OBJECT_TYPE = $SE_FILE_OBJECT)
	...
	Else
		If $_SE_OBJECT_TYPE = $SE_REGISTRY_KEY Then $oName = _Security_RegKeyName($oName)
		$aRet = DllCall($h__Advapi32Dll,'DWORD','GetNamedSecurityInfoW','wstr',$oName,'dword',$_SE_OBJECT_TYPE, _
		'DWORD',$SECURITY_INFORMATION,'ptr',0,'ptr',0,'ptr',0,'ptr',0,'ptr*',0)
	EndIf
	If @error Then Return SetError(@error,0,0)
...
Func  _ConvertSecurityDescriptorToStringSecurityDescriptor(ByRef $pSecDescriptor)
	...
	Local $aRet = DllCall($h__Advapi32Dll,'bool','ConvertSecurityDescriptorToStringSecurityDescriptorW', _
	'ptr',$pSecDescriptor,'DWORD',1,'DWORD',$SECURITY_INFORMATION,'wstr*',0,'ptr',0)
...
Func  _ConvertStringSecurityDescriptorToSecurityDescriptor($strSecDescriptor)
	 If Not IsString($strSecDescriptor) Then Return SetError(1,0,0)
	Local $aRet = DllCall($h__Advapi32Dll,'bool','ConvertStringSecurityDescriptorToSecurityDescriptorW', _
	'wstr',$strSecDescriptor,'DWORD',1,'ptr*',0,'ptr',0)
	If @error Then Return SetError(1,0,0)
	Return $aRet[3]
...
Func _GetSidStruct($AccountName)
	...
	ElseIf StringRegExp($AccountName, '\A(S-\d(-\d+){2,14})\z') Then
...
;                            Sets @error
; Author ........: engine
; Modified.......: FredAI, AdamUL - Added Unicode support.
; Remarks .......:
; Related .......:
; Link ..........;
; Example .......;
...
Func _SetPrivilege($avPrivilege)
	...
	For $i = 0 To $iTokens - 1
		DllCall($h__Advapi32Dll, "int", "LookupPrivilegeValueW", _
			"wstr", "", _
	...
	If $iCount > 0 Then
		...
		For $i = 0 To $iCount - 1
			...
			$avLPN = DllCall($h__Advapi32Dll, "int", "LookupPrivilegeNameW", _
				"wstr", "", _
				"ptr", $pLUID, _
				"ptr", 0, _
				"dword*", 0 )
			$tName = DllStructCreate("wchar[" & $avLPN[4] & "]")
			DllCall($h__Advapi32Dll, "int", "LookupPrivilegeNameW", _
				"wstr", "", _
				"ptr", $pLUID, _
