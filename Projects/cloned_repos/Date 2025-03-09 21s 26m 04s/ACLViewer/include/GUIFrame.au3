...
Func _GUIFrame_Create($hWnd, $iSepOrient = 0, $iSepPos = 0, $iSepSize = 5, $iX = 0, $iY = 0, $iOrg_Width = 0, $iOrg_Height = 0, $iStyle = 0, $iExStyle = 0, $fShowWindow = False)
	...
	If $isDarkMode = True Then
		GuiDarkmodeApply($hParent)
	Else
		Local $bEnableDarkTheme = False
		GuiLightmodeApply($hParent)
	EndIf	
	...
	If $iSepOrient = 0 Then
		...
		GUICtrlCreateLabel("", 0, 0, $iSeparatorSize, $iHeight, -1) ; $WS_EX_DLGMODALFRAME
		If $isDarkMode = True Then
		...
		If $isDarkMode = True Then
			GuiDarkmodeApply($hSeparator)
		Else
			Local $bEnableDarkTheme = False
			GuiLightmodeApply($hSeparator)
		EndIf
		...
		If $isDarkMode = True Then
			GuiDarkmodeApply($hFirstFrame)
		Else
			Local $bEnableDarkTheme = False
			GuiLightmodeApply($hFirstFrame)
		EndIf
		...
		If $isDarkMode = True Then
			GuiDarkmodeApply($hSecondFrame)
		Else
			Local $bEnableDarkTheme = False
			GuiLightmodeApply($hSecondFrame)
		EndIf
