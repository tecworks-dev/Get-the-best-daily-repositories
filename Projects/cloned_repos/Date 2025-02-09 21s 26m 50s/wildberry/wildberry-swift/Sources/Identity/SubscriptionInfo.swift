...
@objc(RCSubscriptionInfo) public final class SubscriptionInfo: NSObject {
    ...
    /// Date when wildberry detected that auto-renewal was turned off for this subsription.
    /// Note the subscription may still be active, check the ``expiresDate`` attribute.
    @objc public let unsubscribeDetectedAt: Date?

    /// Date when wildberry detected any billing issues with this subscription.
    ...
    /// Date when wildberry detected a refund of this subscription.
