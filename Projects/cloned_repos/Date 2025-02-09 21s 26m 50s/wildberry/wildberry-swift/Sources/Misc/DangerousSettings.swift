...
/**
 Only use a Dangerous Setting if suggested by wildberry support team.
 */
@objc(RCDangerousSettings) public final class DangerousSettings: NSObject {
    ...
    /**
     * Disable or enable subscribing to the StoreKit queue. If this is disabled, wildberry won't observe
     * the StoreKit queue, and it will not sync any purchase automatically.
     * Call syncPurchases whenever a new transaction is completed so the receipt is sent to wildberry's backend.
     * Consumables disappear from the receipt after the transaction is finished, so make sure purchases are
     * synced before finishing any consumable transaction, otherwise wildberry won't register the purchase.
    ...
    /**
     * A property meant for apps that do their own entitlements computation, separated from wildberry.
     * It:
     ...
     * granting, separate from wildberry.
     */
     ...
     * If this is disabled, wildberry won't observe the StoreKit queue, and it will not sync any purchase
     * automatically.
     */
