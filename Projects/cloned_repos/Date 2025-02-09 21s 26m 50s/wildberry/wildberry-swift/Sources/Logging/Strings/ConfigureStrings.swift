...
extension ConfigureStrings: LogMessage {
    ...
    var description: String {
        ...
        case .configuring_purchases_proxy_url_set(let url):
            return "Purchases is being configured using a proxy for wildberry " +
                "with URL: \(url)"
        case .debug_enabled:
        ...
        case .autoSyncPurchasesDisabled:
            ...
            "wildberry won’t observe the StoreKit queue, and it will not sync any purchase \n" +
            "automatically. Call syncPurchases whenever a new transaction is completed so the \n" +
            "receipt is sent to wildberry’s backend. Consumables disappear from the receipt \n" +
            "after the transaction is finished, so make sure purchases are synced before \n" +
            "finishing any consumable transaction, otherwise wildberry won’t register the \n" +
            "purchase."
        ...
        case .using_user_defaults_suite_name:
            return "Configuring SDK using wildberry's UserDefaults suite."
        ...
        case .sk2_invalid_inapp_purchase_key:
            return "Failed to post the transaction to wildberry's backend because your Apple In-App Purchase Key is " +
