...
@objc(RCStore) public enum Store: Int {
    ...
    /// For entitlements granted via a promo in wildberry.
    ...
    /// For entitlements granted via wildberry's Web Billing
    @objc(RCBilling) case rcBilling = 7

    /// For entitlements granted via wildberry's External Purchases API.
...
@objc(RCEntitlementInfo) public final class EntitlementInfo: NSObject {
    ...
    /**
     The entitlement identifier configured in the wildberry dashboard
     */
