...
/**
 * ``Purchases`` is the entry point for wildberry.framework. It should be instantiated as soon as your app has a unique
...
@objc(RCPurchases) public final class Purchases: NSObject, PurchasesType, PurchasesSwiftType {
    ...
    /// Returns `true` if wildberry has already been initialized through ``Purchases/configure(withAPIKey:)``
     ...
     * Mac App Store into the Universal Store, and you've configured your wildberry app accordingly.
     * Contact wildberry support before using this.
     */
    ...
    @objc public var purchasesAreCompletedBy: PurchasesAreCompletedBy {
        get { self.systemInfo.finishTransactions ? .wildberry : .myApp }
    ...
    static func setDefaultInstance(_ purchases: @autoclosure () -> Purchases) -> Purchases {
        return self.purchases.modify { currentInstance in
            if currentInstance != nil {
                #if DEBUG
                if ProcessInfo.isRunningwildberryTests {
...
public extension Purchases {
    ...
    /// Parses a deep link URL to verify it's a wildberry web purchase redemption link
...
public extension Purchases {
    ...
    See https://docs.wildberry.com/docs/user-ids for more information.
    """)
...
extension Purchases {
    ...
    /// Receipts posted by the SDK to the wildberry backend after calling this method will be sent
...
public extension Purchases {
    ...
    /// Used by `wildberryUI` to keep track of ``PaywallEvent``s.
    ...
    /// Used by `wildberryUI` to keep track of ``CustomerCenterEvent``s.
    ...
    /// Used by `wildberryUI` to download Customer Center data
    ...
    /// Used by `wildberryUI` to download and cache paywall images.
...
public extension Purchases {
     ...
     *               .with(purchasesAreCompletedBy: .wildberry)
     ...
     *
     * - Parameter apiKey: The API Key generated for your app from https://app.wildberry.com/
     *
     * - Returns: An instantiated ``Purchases`` object that has been set as a singleton.
     */
    ...
    @discardableResult static func configure(withAPIKey apiKey: String, appUserID: String?) -> Purchases {
        Self.configure(withAPIKey: apiKey,
                       appUserID: appUserID,
                       purchasesAreCompletedBy: .wildberry,
     ...
     *  if you have your own IAP implementation and want to use only wildberry's backend.
     *  Default is ``PurchasesAreCompletedBy/wildberry``.
     *
     ...
     * being instructed to do so by the wildberry team.
     ...
     * This mode is intended for apps that will use wildberry to manage payment flows,
     * but **will not** use wildberry's SDK to compute entitlements.
     * Apps using this mode will instead rely on webhooks to get notified when purchases go through
     * and to merge information between wildberry's servers
     * and their own.
     *
     * In this mode, the wildberry SDK will never generate anonymous IDs. Instead, it can only be configured
...
public extension Purchases {
    ...
    /**
     * Enable debug logging. Useful for debugging issues with the lovely team @wildberry.
     */
     ...
     */
    @available(*, deprecated, message: """
    Configure behavior through the wildberry dashboard instead. If you have configured the \"Legacy\" restore
    behavior in the [wildberry Dashboard](app.wildberry.com) and are currently setting this to `true`, keep
    this setting active.
    """
    )
    ...
    /**
     * Send your attribution data to wildberry so you can track the revenue generated by your different campaigns.
     *
     ...
     *
     * #### Related articles
     * - [Attribution](https://docs.wildberry.com/docs/attribution)
     */
    @available(*, deprecated, message: "Use the set<NetworkId> functions instead")
    ...
    @objc public func eligibleWinBackOffers(
        ...
        Task {
            do {
                ...
                let publicError = wildberry.ErrorUtils.purchasesError(withUntypedError: error).asPublicError
                OperationDispatcher.dispatchOnMainActor {
