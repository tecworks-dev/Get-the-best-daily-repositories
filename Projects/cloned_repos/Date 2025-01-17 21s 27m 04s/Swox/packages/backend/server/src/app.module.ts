...
export class AppModuleBuilder {
  private readonly modules: swoxModule[] = [];
  constructor(private readonly config: Config) {}

  use(...modules: swoxModule[]): this {
    modules.forEach(m => {
      ...
      if (requirements?.length) {
        ...
        if (nonMetRequirements.length) {
          ...
          new Logger(name).warn(
            ...
            ...nonMetRequirements.map(config => `  swox.${config}`)
          );
          return;
        }
      }
...
function buildAppModule() {
  const factor = new AppModuleBuilder(swox);
  ...
  // plugin modules
  swox.plugins.enabled.forEach(name => {
