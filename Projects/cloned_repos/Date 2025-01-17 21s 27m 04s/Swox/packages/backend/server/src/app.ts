...
export async function createApp() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    rawBody: true,
    bodyParser: true,
    logger: swox.swox.stable ? ['log'] : ['verbose'],
  });
  ...
  if (swox.isSelfhosted && swox.telemetry.enabled) {
    const mixpanel = await import('mixpanel');
    mixpanel.init(swox.telemetry.token).track('selfhost-server-started', {
      version: swox.version,
    });
  }
