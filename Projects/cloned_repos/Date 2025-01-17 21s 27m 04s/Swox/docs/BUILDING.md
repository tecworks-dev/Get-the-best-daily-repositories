...
swox client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/swoxapp/swox
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS. [see problem here](https://github.com/swoxapp/swox/discussions/2840)
...
Also make sure you have built the `@swox/core` workspace before running E2E tests.
...
```shell
# there are `swox-local`, `swox-migration`, `swox-local`, `swox-prototype` e2e tests,
