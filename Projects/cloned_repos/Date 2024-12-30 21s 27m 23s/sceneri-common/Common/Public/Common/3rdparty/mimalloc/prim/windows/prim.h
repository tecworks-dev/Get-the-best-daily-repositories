...
void _mi_prim_mem_init(mi_os_mem_config_t* config)
{
	...
	HINSTANCE hDll;
	hDll = LoadLibraryW(TEXT("kernelbase.dll"));
	if (hDll != NULL)
	{
	...
	hDll = LoadLibraryW(TEXT("ntdll.dll"));
	if (hDll != NULL)
	{
	...
	hDll = LoadLibraryW(TEXT("kernel32.dll"));
	if (hDll != NULL)
	{
...
static void*
win_virtual_alloc(void* addr, size_t size, size_t try_alignment, DWORD flags, bool large_only, bool allow_large, bool* is_large)
{
	mi_assert_internal(!(large_only && !allow_large));
	static MiAtomic(size_t) large_page_try_ok; // = 0;
	void* p = NULL;
	// Try to allocate large OS pages (2MiB) if allowed or required.
	if ((large_only || _mi_os_use_large_page(size, try_alignment)) && allow_large && (flags & MEM_COMMIT) != 0 && (flags & MEM_RESERVE) != 0)
	{
	...
	// if (p == NULL) { _mi_warning_message("unable to allocate OS memory (%zu bytes, error code: 0x%x, address: %p, alignment: %zu, flags:
	// 0x%x, large only: %d, allow large: %d)\n", size, GetLastError(), addr, try_alignment, flags, large_only, allow_large); }
	return p;
}
...
void _mi_prim_process_info(mi_process_info_t* pinfo)
{
	...
	{
		HINSTANCE hDll = LoadLibraryW(TEXT("psapi.dll"));
		if (hDll != NULL)
		{
...
bool _mi_prim_random_buf(void* buf, size_t buf_len)
{
	...
	{
		HINSTANCE hDll = LoadLibraryW(TEXT("bcrypt.dll"));
		if (hDll != NULL)
		{
