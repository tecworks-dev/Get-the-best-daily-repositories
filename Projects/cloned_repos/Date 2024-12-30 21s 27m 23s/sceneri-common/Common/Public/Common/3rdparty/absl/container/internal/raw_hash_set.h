...
#include "Common/3rdparty/absl/base/internal/endian.h"
#include "Common/3rdparty/absl/base/optimization.h"
#include "Common/3rdparty/absl/base/port.h"
#include "Common/3rdparty/absl/container/internal/common.h"
#include "Common/3rdparty/absl/container/internal/compressed_tuple.h"
#include "Common/3rdparty/absl/container/internal/container_memory.h"
#include "Common/3rdparty/absl/container/internal/hash_policy_traits.h"
#include "Common/3rdparty/absl/container/internal/hashtable_debug_hooks.h"
#include "Common/3rdparty/absl/container/internal/hashtablez_sampler.h"
#include "Common/3rdparty/absl/container/internal/have_sse.h"
#include "Common/3rdparty/absl/container/internal/layout.h"
#include "Common/3rdparty/absl/memory/memory.h"
...
namespace absl
{
	...
	namespace container_internal
	{
		...
		struct IsDecomposable<
			absl::void_t<decltype(Policy::apply(RequireUsableKey<typename Policy::key_type, Hash, Eq>(), std::declval<Ts>()...))>,
			Policy,
			Hash,
			Eq,
			Ts...> : std::true_type
		...
		{
			return sizeof(T) == 8 ? base_internal::CountTrailingZerosNonZero64(static_cast<uint64_t>(x))
		...
		{
			return sizeof(T) == 8 ? base_internal::CountLeadingZeros64(static_cast<uint64_t>(x))
		...
		public:
			...
			{
				return (sizeof(T) * CHAR_BIT - container_internal::LeadingZeros(mask_) - 1) >> Shift;
			}
		...
		private:
			...
			using RequiresInsertable =
				typename std::enable_if<absl::disjunction<std::is_convertible<T, init_type>, SameAsElementReference<T>>::value, int>::type;
		...
		public:
			...
			private:
				...
				void skip_empty_or_deleted()
				{
					...
					if (ABSL_PREDICT_FALSE(*ctrl_ == kSentinel))
						ctrl_ = nullptr;
				}
			...
			raw_hash_set(
			) noexcept(std::is_nothrow_default_constructible<hasher>::value&& std::is_nothrow_default_constructible<key_equal>::value&&
			...
			) noexcept(std::is_nothrow_copy_constructible<hasher>::value&& std::is_nothrow_copy_constructible<key_equal>::value&&
			...
			) noexcept(absl::allocator_traits<allocator_type>::is_always_equal::value&& std::is_nothrow_move_assignable<hasher>::value&&
			...
			void drop_deletes_without_resize() ABSL_ATTRIBUTE_NOINLINE
			{
				...
				for (size_t i = 0; i != capacity_; ++i)
				{
					if (!IsDeleted(ctrl_[i]))
						continue;
					size_t hash = PolicyTraits::apply(HashElement{hash_ref()}, PolicyTraits::element(slots_ + i));
					auto target = find_first_non_full(hash);
					size_t new_i = target.offset;
					total_probe_length += target.probe_length;
		...
		protected:
			...
			void emplace_at(size_t i, Args&&... args)
			{
				...
				assert(
					PolicyTraits::apply(FindElement{*this}, *iterator_at(i)) == iterator_at(i) && "constructed value does not match the lookup key"
				);
			}
		...
		private:
			...
			absl::container_internal::CompressedTuple<size_t /* growth_left */, hasher, key_equal, allocator_type> settings_{
