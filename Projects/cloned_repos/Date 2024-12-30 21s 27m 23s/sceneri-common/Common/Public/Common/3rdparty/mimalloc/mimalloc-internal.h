...
#include "Common/3rdparty/mimalloc/mimalloc-types.h"
#include "Common/3rdparty/mimalloc/mimalloc-track.h"
...
void* _mi_os_alloc_aligned_at_offset(
	size_t size, size_t alignment, size_t align_offset, bool commit, bool allow_large, mi_memid_t* memid, mi_stats_t* tld_stats
);
...
void* _mi_os_alloc_huge_os_pages(
	size_t pages, int numa_node, mi_msecs_t max_secs, size_t* pages_reserved, size_t* psize, mi_memid_t* memid
);
...
static inline mi_segment_t* _mi_page_segment(const mi_page_t* page)
{
	...
	mi_assert_internal(
		segment == NULL || ((mi_slice_t*)page >= segment->slices && (mi_slice_t*)page < segment->slices + segment->slice_entries)
	);
	return segment;
}
...
extern MiAtomic(size_t) _mi_numa_node_count;
