...
public:
	...
	explicit GenericValue(const GenericValue& rhs, Allocator& allocator)
	{
		switch (rhs.GetType())
		{
			case kObjectType:
				CopyObjectRaw(rhs.GetMembersPointer(), rhs.MemberCount(), allocator);
				break;
			case kArrayType:
				CopyArrayRaw(rhs.GetArray().begin(), rhs.Size(), allocator);
				break;
			case kStringType:
				SetStringRaw(StringRefType(rhs.GetString(), rhs.GetStringLength()), allocator);
				break;
			default:
...
public:
	...
	*/
	...
	//! Get the maximum capacity of members in the object.
	...
	*/
	Member& AddMember(GenericValue&& name, GenericValue&& value, Allocator& allocator)
	{
		RAPIDJSON_ASSERT(IsObject());
		RAPIDJSON_ASSERT(name.IsString());
		...
		if (o.size >= o.capacity)
		{
			...
			else
			{
				...
				SetMembersPointer(
					reinterpret_cast<Member*>(allocator.Realloc(GetMembersPointer(), oldCapacity * sizeof(Member), o.capacity * sizeof(Member)))
				);
			}
		}
		Member& member = GetMembersPointer()[o.size];

		new (&member.name) GenericValue(Forward<GenericValue>(name));
		new (&member.value) GenericValue(Forward<GenericValue>(value));
		o.size++;
		return member;
	}
	...
	/*! \param name A string value as name of member.
	    ...
	    \param allocator    Allocator for reallocating memory. It must be the same one as used before. Commonly use
	   GenericDocument::GetAllocator(). \return The newly inserted value \pre  IsObject() \note This overload is needed to avoid clashes with
	   the generic primitive type AddMember(GenericValue&,T,Allocator&) overload below. \note Amortized Constant time complexity.
	*/
	Member& AddMember(GenericValue&& name, StringRefType value, Allocator& allocator)
	{
		return AddMember(Forward<GenericValue>(name), GenericValue(value), allocator);
	}
	...
	/*! \param name A string value as name of member.
	   ...
	   the generic primitive type AddMember(GenericValue&,T,Allocator&) overload below. \note Amortized Constant time complexity.
	*/
	Member& AddMember(GenericValue& name, std::basic_string<Ch>& value, Allocator& allocator)
	{
	...
	*/
	template<typename T>
	RAPIDJSON_DISABLEIF_RETURN((internal::OrExpr<internal::IsPointer<T>, internal::IsGenericValue<T>>), (Member&))
	...
	/*! \param name A constant string reference as name of member.
	    \param value Value of any type.
	    \param allocator    Allocator for reallocating memory. It must be the same one as used before. Commonly use
	   GenericDocument::GetAllocator(). \return The newly inserted value \note The ownership of \c value will be transferred to this object on
	   success. \pre  IsObject() \post value.IsNull() \note Amortized Constant time complexity.
	*/
	Member& AddMember(StringRefType name, GenericValue&& value, Allocator& allocator)
	{
		return AddMember(GenericValue(name), Forward<GenericValue>(value), allocator);
	}
	...
	/*! \param name A constant string reference as name of member.
	    ...
	    \param allocator    Allocator for reallocating memory. It must be the same one as used before. Commonly use
	   GenericDocument::GetAllocator(). \return The newly inserted value \pre  IsObject() \note This overload is needed to avoid clashes with
	   the generic primitive type AddMember(StringRefType,T,Allocator&) overload below. \note Amortized Constant time complexity.
	*/
	Member& AddMember(StringRefType name, StringRefType value, Allocator& allocator)
	{
		return AddMember(name, GenericValue(value), allocator);
	}
	...
	*/
	...
	{
		return AddMember(GenericValue(name), value, allocator);
	}
	...
	*/
	...
	void ReserveMembers(SizeType newCapacity, Allocator& allocator)
	{
		RAPIDJSON_ASSERT(IsObject());
		RAPIDJSON_ASSERT(newCapacity >= data_.o.size);
		if (newCapacity > data_.o.capacity)
		{
			SetMembersPointer(
				reinterpret_cast<Member*>(allocator.Realloc(GetMembersPointer(), data_.o.capacity * sizeof(Member), newCapacity * sizeof(Member)))
			);
	...
	*/
	MemberIterator RemoveMember(MemberIterator m)
	{
		...
		if (data_.o.size > 1 && m != last)
			*m = Move(*last); // Move the last one to this place
		else
	...
	*/
	...
	/*! \param value        Value to be appended.
	    \param allocator    Allocator for reallocating memory. It must be the same one as used before. Commonly use
	   GenericDocument::GetAllocator(). \pre IsArray() == true \post value.IsNull() == true \return The newly inserted elemnet \note The
	   ownership of \c value will be transferred to this array on success. \note If the number of elements to be appended is known, calls
	...
	*/
	GenericValue& PushBack(GenericValue&& value, Allocator& allocator)
	{
		...
		GenericValue* pAddress = &GetElementsPointer()[data_.a.size++];
		new (pAddress) GenericValue(Forward<GenericValue>(value));
		return *pAddress;
	}
	...
	/*! \param value        Constant string reference to be appended.
	   ...
	   GenericDocument::GetAllocator(). \pre IsArray() == true \return The newly inserted elemnet \note If the number of elements to be
	   appended is known, calls Reserve() once first may be more efficient. \note Amortized constant time complexity. \see GenericStringRef
	*/
	...
	*/
	...
	{
		return PushBack(GenericValue(value), allocator);
	}
...
private:
	...
	void SetArrayRaw(const GenericValue* values, SizeType count, Allocator& allocator)
	{
	...
	void SetObjectRaw(const Member* members, SizeType count, Allocator& allocator)
	{
	...
	// Initialize this value as array with initial data, without calling destructor.
	void CopyArrayRaw(const GenericValue* values, SizeType count, Allocator& allocator)
	{
		data_.f.flags = kArrayFlag;
		data_.a.capacity = count;
		if (count)
		{
			GenericValue* e = static_cast<GenericValue*>(allocator.Malloc(count * sizeof(GenericValue)));
			SetElementsPointer(e);
			...
			{
				PushBack(GenericValue(values[i], allocator), allocator);
			}
		}
		else
		{
	...
	//! Initialize this value as object with initial data, without calling destructor.
	void CopyObjectRaw(const Member* members, SizeType count, Allocator& allocator)
	{
		data_.f.flags = kObjectFlag;
		data_.o.capacity = count;
		if (count)
		{
			Member* m = static_cast<Member*>(allocator.Malloc(count * sizeof(Member)));
			SetMembersPointer(m);
			...
			for (SizeType i = 0; i < count; ++i)
			{
				const StringRefType name = StringRefType(members[i].name.GetString(), members[i].name.GetStringLength());
				AddMember(GenericValue(name, allocator), GenericValue(members[i].value, allocator), allocator);
			}
		}
		else
		{
...
public:
	...
	*/
	...
	GenericDocument(GenericDocument&& rhs) RAPIDJSON_NOEXCEPT
		: ValueType(std::move(static_cast<ValueType&>(rhs))), // explicit cast to avoid prohibited move from Document
	...
	GenericDocument(const GenericDocument& rhs) RAPIDJSON_NOEXCEPT : ValueType(static_cast<const ValueType&>(rhs), *rhs.allocator_),
																																	 allocator_(rhs.allocator_),
																																	 ownAllocator_(0),
																																	 stack_(rhs.stack_),
																																	 parseResult_(rhs.parseResult_)
	...
	GenericDocument& operator=(const GenericDocument& rhs) RAPIDJSON_NOEXCEPT
	{
		// Calling the destructor here would prematurely call stack_'s destructor
		Destroy();
		...
		const ValueType& thisValue = static_cast<const ValueType&>(rhs);
		ValueType newValue(thisValue, *allocator_);
		ValueType::operator=(std::move(newValue));
	...
	*/
	...
	GenericDocument& ParseStream(InputStream& is)
	{
		...
		{
			RAPIDJSON_ASSERT(stack_.GetSize() == sizeof(ValueType));        // Got one and only one root object
			ValueType::operator=(Move(*stack_.template Pop<ValueType>(1))); // Move value from stack to document
		}
		return *this;
	}
	...
	*/
	...
	{
		RAPIDJSON_ASSERT(allocator_ != nullptr);
		return *allocator_;
	}
...
extern template class GenericDocument<UTF8<>, MemoryPoolAllocator<>, CrtAllocator>;
extern template class GenericValue<UTF8<>, MemoryPoolAllocator<>>;
extern template struct GenericMember<UTF8<>, MemoryPoolAllocator<CrtAllocator>>;
extern template struct GenericStringRef<char>;
extern template class GenericMemberIterator<true, UTF8<>, MemoryPoolAllocator<CrtAllocator>>;
