...
{
	return (int)syscall(SYS_open, fpath, open_flags, 0);
}
...
{
	return (int)syscall(SYS_access, fpath, mode);
}

#elif !defined(__APPLE__) && !defined(__EMSCRIPTEN__) // avoid unused warnings
	...
	if (addr == NULL && try_alignment > 1 && (try_alignment % _mi_os_page_size()) == 0)
	{
		...
		{ // alignment is a power of 2 and 4096 <= alignment <= 1GiB
			...
			if (p == MAP_FAILED || !_mi_is_aligned(p, try_alignment))
			{
				int err = errno;
				_mi_warning_message(
					"unable to directly request aligned OS memory (error: %d (0x%x), size: 0x%zx bytes, alignment: 0x%zx, hint address: %p)\n",
					err,
					err,
					size,
					try_alignment,
...
#define MAP_NORESERVE 0
	...
	{
		static MiAtomic(size_t) large_page_try_ok; // = 0;
...
// default `MADV_DONTNEED` is used though.
	...
	static MiAtomic(size_t) advice = MI_ATOMIC_VAR_INIT(MADV_FREE);
...
static long
mi_prim_mbind(void* start, unsigned long len, unsigned long mode, const unsigned long* nmask, unsigned long maxnode, unsigned flags)
{
...
#elif defined(__ANDROID__) || defined(__DragonFly__) || defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__) || \
	defined(__sun)
...
#endif
	static MiAtomic(uintptr_t) no_getrandom; // = 0
