...
#include "Common/3rdparty/mimalloc/mimalloc-atomic.h" // MiAtomic
...
typedef struct mi_page_s
{
	...
	MiAtomic(mi_thread_free_t) xthread_free; // list of deferred free blocks freed by other threads
	MiAtomic(uintptr_t) xheap;
...
typedef struct mi_segment_s
{
	...
	MiAtomic(struct mi_segment_s*) abandoned_next;
	...
	MiAtomic(mi_threadid_t) thread_id; // unique id of the thread owning this segment
...
{
	mi_tld_t* tld;
	mi_page_t* pages_free_direct[MI_PAGES_DIRECT]; // optimize: array where every entry points a page with possibly free blocks in the
	                                               // corresponding queue for that size.
	mi_page_queue_t pages[MI_BIN_FULL + 1];        // queue of pages for each size class (or "bin")
	MiAtomic(mi_block_t*) thread_delayed_free;
...
#if (MI_DEBUG)
#define mi_assert(expr) Assert(expr)
#else
#define mi_assert(x)
#endif
