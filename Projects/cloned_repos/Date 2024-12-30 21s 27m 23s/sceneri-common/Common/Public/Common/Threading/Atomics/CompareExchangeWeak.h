...
namespace ngine::Threading::Atomics
{
	...
	FORCE_INLINE bool CompareExchangeWeak(Type& value, Type& expected, Type desired)
	{
#if COMPILER_MSVC
...
#elif COMPILER_CLANG || COMPILER_GCC
		return __atomic_compare_exchange(&value, &expected, &desired, true, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
#else
		return reinterpret_cast<std::atomic<Type>&>(value).compare_exchange_weak(expected, desired);
#endif
	}

	template<typename Type>
	FORCE_INLINE bool CompareExchangeWeak(Type*& value, Type*& expected, Type* const desired)
	{
		return CompareExchangeWeak<uintptr>(
