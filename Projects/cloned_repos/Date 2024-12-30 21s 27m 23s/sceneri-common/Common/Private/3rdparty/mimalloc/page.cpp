...
#define MI_IN_PAGE_C
#include "Common/3rdparty/mimalloc/page-queue.h"
#undef MI_IN_PAGE_C
...
bool _mi_page_is_valid(mi_page_t* page)
{
	...
	if (mi_page_heap(page) != NULL)
	{
...
#endif
		{
			...
			mi_assert_internal(
				pq->block_size == mi_page_block_size(page) || mi_page_block_size(page) > MI_MEDIUM_OBJ_SIZE_MAX || mi_page_is_in_full(page)
			);
...
void _mi_page_retire(mi_page_t* page) mi_attr_noexcept
{
	...
	mi_page_queue_t* pq = mi_page_queue_of(page);
	if mi_likely (page->xblock_size <= MI_MAX_RETIRE_SIZE && !mi_page_queue_is_special(pq))
...
static void mi_page_free_list_extend_secure(
	mi_heap_t* const heap, mi_page_t* const page, const size_t bsize, const size_t extend, mi_stats_t* const stats
)
{
	MI_UNUSED(stats);
#if (MI_SECURE <= 2)
...
mi_page_free_list_extend(mi_page_t* const page, const size_t bsize, const size_t extend, mi_stats_t* const stats)
{
	MI_UNUSED(stats);
#if (MI_SECURE <= 2)
	mi_assert_internal(page->free == NULL);
...
static void mi_page_init(mi_heap_t* heap, mi_page_t* page, size_t block_size, mi_tld_t* tld)
{
	...
	page->xblock_size =
		(block_size < MI_HUGE_BLOCK_SIZE ? (uint32_t)block_size : MI_HUGE_BLOCK_SIZE); // initialize before _mi_segment_page_start
	size_t page_size;
...
static MiAtomic(void*) deferred_arg; // = NULL
...
#else
	mi_page_queue_t* pq =
		mi_page_queue(heap, is_huge ? MI_HUGE_BLOCK_SIZE : block_size); // not block_size as that can be low if the page_alignment > 0
