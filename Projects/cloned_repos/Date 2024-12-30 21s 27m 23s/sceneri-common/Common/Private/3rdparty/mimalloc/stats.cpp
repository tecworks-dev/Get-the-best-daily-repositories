...
static void mi_printf_amount(int64_t n, int64_t unit, mi_output_fun* out, void* arg, const char* fmt)
{
	...
	{
		if (n != 1 || suffix[0] != 'B')
		{ // skip printing 1 B for the unit column
...
static void _mi_stats_print(mi_stats_t* stats, mi_output_fun* out0, void* arg0) mi_attr_noexcept
{
	...
	mi_stat_print(
		&stats->normal,
		"normal",
		(stats->normal_count.count == 0 ? 1 : -(stats->normal.allocated / stats->normal_count.count)),
		out,
		arg
	);
	mi_stat_print(
...
) mi_attr_noexcept
{
	...
	pinfo.current_commit = (size_t)(mi_atomic_loadi64_relaxed((MiAtomic(int64_t)*)&_mi_stats_main.committed.current));
	pinfo.peak_commit = (size_t)(mi_atomic_loadi64_relaxed((MiAtomic(int64_t)*)&_mi_stats_main.committed.peak));
