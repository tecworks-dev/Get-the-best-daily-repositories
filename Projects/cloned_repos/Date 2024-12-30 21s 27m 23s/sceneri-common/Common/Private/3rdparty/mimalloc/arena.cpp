...
DISABLE_CLANG_WARNING("-Wbad-function-cast")
DISABLE_CLANG_WARNING("-Watomic-implicit-seq-cst")
...
#include "Common/3rdparty/mimalloc/bitmap.h" // atomic bitmap
...
typedef struct mi_arena_s
{
	...
	MiAtomic(uint8_t*) start;             // the start of the memory area
	...
	MiAtomic(size_t) search_idx;          // optimization to start the search for free blocks
	MiAtomic(mi_msecs_t) purge_expire;    // expiration time when blocks should be decommitted from `blocks_decommit`.
...
static mi_decl_cache_align MiAtomic(mi_arena_t*) mi_arenas[MI_MAX_ARENAS];
static mi_decl_cache_align MiAtomic(size_t) mi_arena_count; // = 0

// static bool mi_manage_os_memory_ex2(void* start, size_t size, bool is_large, int numa_node, bool exclusive, mi_memid_t memid,
...
static MiAtomic(size_t) mi_arena_static_top;
...
{
	size_t idx =
		0; // mi_atomic_load_relaxed(&arena->search_idx);  // start from last search; ok to be relaxed as the exact start does not matter
...
mi_arena_try_alloc_at(mi_arena_t* arena, size_t arena_index, size_t needed_bcount, bool commit, mi_memid_t* memid, mi_os_tld_t* tld)
{
...
static void* mi_arena_try_alloc_at_id(
...
)
{
	...
	{ // in not specific, check numa affinity
...
)
{
	...
	else
	{
		...
		{
			void* p =
				mi_arena_try_alloc_at_id(mi_arena_id_create(i), true, numa_node, size, alignment, commit, allow_large, req_arena_id, memid, tld);
			if (p != NULL)
				return p;
		}
		...
		if (numa_node >= 0)
		{ // if numa_node was < 0 (no specific affinity requested), all arena's have been tried already
			for (size_t i = 0; i < max_arena; i++)
			{
...
static bool mi_arena_try_purge(mi_arena_t* arena, mi_msecs_t now, bool force, mi_stats_t* stats)
{
	...
	mi_atomic_casi64_strong_acq_rel(&arena->purge_expire, &expire, long(0));
...
void _mi_arena_free(void* p, size_t size, size_t committed_size, mi_memid_t memid, mi_stats_t* stats)
{
	...
	else if (memid.memkind == MI_MEM_ARENA)
	{
		...
		if (arena == NULL)
		{
			_mi_error_message(EINVAL, "trying to free from non-existent arena: %p, size %zu\n", p, size);
			return;
		}
		mi_assert_internal(arena->field_count > mi_bitmap_index_field(bitmap_idx));
		if (arena->field_count <= mi_bitmap_index_field(bitmap_idx))
		{
			_mi_error_message(EINVAL, "trying to free from non-existent arena block: %p, size %zu\n", p, size);
			return;
		}
...
static bool mi_manage_os_memory_ex2(
	void* start, size_t size, bool is_large, int numa_node, bool exclusive, mi_memid_t memid, mi_arena_id_t* arena_id
) mi_attr_noexcept
{
	if (arena_id != NULL)
