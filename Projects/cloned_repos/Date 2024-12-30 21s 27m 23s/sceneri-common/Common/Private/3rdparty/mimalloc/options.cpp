...
DISABLE_CLANG_WARNING("-Wtautological-unsigned-enum-zero-compare")
...
#include "Common/3rdparty/mimalloc/mimalloc-prim.h" // mi_prim_out_stderr
...
// stable options
	...
	{2, UNINIT, MI_OPTION_LEGACY(arena_eager_commit, eager_region_commit)}, // eager commit arena's? 2 is used to enable this only on an OS
                                                                          // that has overcommit (i.e. linux)
	{1, UNINIT, MI_OPTION_LEGACY(purge_decommits, reset_decommits)}, // purge decommits memory (instead of reset) (note: on linux this uses
                                                                   // MADV_DONTNEED for decommit)
	{0, UNINIT, MI_OPTION_LEGACY(allow_large_os_pages, large_os_pages)}, // use large OS pages, use only with eager commit to prevent
	...
	{1,
   UNINIT,
   MI_OPTION(eager_commit_delay)}, // the first N segments per thread are not eagerly committed (but per page in the segment on demand)
#endif
...
static MiAtomic(size_t) out_len;
...
static MiAtomic(void*) mi_out_arg;              // = NULL
...
// --------------------------------------------------------
static MiAtomic(size_t) error_count;   // = 0;  // when >= max_error_count stop emitting errors
static MiAtomic(size_t) warning_count; // = 0;  // when >= max_warning_count stop emitting warnings
...
static MiAtomic(void*) mi_error_arg;             // = NULL
