...
#include "Common/3rdparty/mimalloc/mimalloc-prim.h"
...
#if (MI_INTPTR_SIZE >= 8)
static mi_decl_cache_align MiAtomic(uintptr_t) aligned_base;
...
void* _mi_os_get_aligned_hint(size_t try_alignment, size_t size)
{
	...
	if (size > 1 * MI_GiB)
		return NULL; // guarantee the chance of fixed valid address is at most 1/(MI_HINT_AREA / 1<<30) = 1/4096.
#if (MI_SECURE > 0)
...
static void*
mi_os_prim_alloc(size_t size, size_t try_alignment, bool commit, bool allow_large, bool* is_large, bool* is_zero, mi_stats_t* stats)
{
	...
	*is_zero = false;
	...
	{
		_mi_warning_message(
			"unable to allocate OS memory (error: %d (0x%x), size: 0x%zx bytes, align: 0x%zx, commit: %d, allow large: %d)\n",
			err,
			err,
			size,
			try_alignment,
...
static void* mi_os_prim_alloc_aligned(
	size_t size, size_t alignment, bool commit, bool allow_large, bool* is_large, bool* is_zero, void** base, mi_stats_t* stats
)
{
	...
	else
	{
		...
		_mi_warning_message(
			"unable to allocate aligned OS memory directly, fall back to over-allocation (size: 0x%zx bytes, address: %p, alignment: 0x%zx, "
			"commit: %d)\n",
			size,
			p,
			alignment,
		...
		{ // win32 virtualAlloc cannot free parts of an allocate block
...
void* _mi_os_alloc_aligned(size_t size, size_t alignment, bool commit, bool allow_large, mi_memid_t* memid, mi_stats_t* tld_stats)
{
	...
	*memid = _mi_memid_none();
	...
	void* os_base = NULL;
	void* p =
		mi_os_prim_alloc_aligned(size, alignment, commit, allow_large, &os_is_large, &os_is_zero, &os_base, &_mi_stats_main /*tld->stats*/);
	if (p != NULL)
	{
...
void* _mi_os_alloc_aligned_at_offset(
	size_t size, size_t alignment, size_t offset, bool commit, bool allow_large, mi_memid_t* memid, mi_stats_t* tld_stats
)
{
...
static mi_decl_cache_align MiAtomic(uintptr_t) mi_huge_start; // = 0
...
void* _mi_os_alloc_huge_os_pages(
	size_t pages, int numa_node, mi_msecs_t max_msecs, size_t* pages_reserved, size_t* psize, mi_memid_t* memid
)
{
	*memid = _mi_memid_none();
	if (psize != NULL)
	...
	while (page < pages)
	{
		...
		{
			_mi_warning_message(
				"unable to allocate huge OS page (error: %d (0x%x), address: %p, size: %zx bytes)\n",
				err,
				err,
				addr,
...
MiAtomic(size_t) _mi_numa_node_count; // = 0   // cache the node count
