...
#include "Common/3rdparty/mimalloc/mimalloc-prim.h" // _mi_prim_thread_id()

#include <string.h> // memset, strlen (for mi_strdup)
#include <stdlib.h> // malloc, abort

#define MI_IN_ALLOC_C
#include "Common/3rdparty/mimalloc/alloc-override.h"
#undef MI_IN_ALLOC_C
...
static mi_decl_noinline void _mi_free_block_mt(mi_page_t* page, mi_block_t* block)
{
	...
	if (segment->kind != MI_SEGMENT_HUGE)
	{ // not for huge segments as we just reset the content
...
static inline mi_segment_t* mi_checked_ptr_segment(const void* p, const char* msg)
{
	...
	if mi_unlikely (!mi_is_in_heap_region(p))
	{
		...
		{ // linux tends to align large blocks above 0x7F000000000 (issue #640)
#else
		{
#endif
			_mi_warning_message(
				"%s: pointer might not point to a valid heap region: %p\n"
...
void mi_free(void* p) mi_attr_noexcept
{
	...
	{                                              // thread-local free?
		if mi_likely (page->flags.full_aligned == 0) // and it is not a full page (full pages need to move from the full bin), nor has aligned
		                                             // blocks (aligned blocks need to be unaligned)
		{
...
mi_page_usable_aligned_size_of(const mi_segment_t* segment, const mi_page_t* page, const void* p) mi_attr_noexcept
{
...
void* _mi_heap_realloc_zero(mi_heap_t* heap, void* p, size_t newsize, bool zero) mi_attr_noexcept
{
	...
	{ // note: newsize must be > 0 or otherwise we return NULL for realloc(NULL,0)
...
#ifdef __cplusplus
#include <new>
static bool mi_try_new_handler()
{
...
mi_decl_export mi_decl_noinline void* mi_heap_try_new(mi_heap_t* heap, size_t size)
{
	void* p = NULL;
	while (p == NULL && mi_try_new_handler())
	{
...
static mi_decl_noinline void* mi_try_new(size_t size)
{
	return mi_heap_try_new(mi_prim_get_default_heap(), size);
}
...
mi_decl_nodiscard void* mi_new_realloc(void* p, size_t newsize)
{
	...
	} while (q == NULL && mi_try_new_handler());
	return q;
}
