...
def main() -> None:
    ...
    # Check astra authorization header
    ...
    if "result" not in result_json or "quota_limit" not in result_json["result"]:
        print(f"Failed to check astra account status with result {result_json} returned. Please provide correct "
              f"astra authorization header with argument --blxr-auth-header and try again.")
        sys.exit(0)
    ...
    print("---------------------------------------------------------------------------------------------------------")
    print("Summary:")
    print(f"Sent {num_tx_groups} groups of transactions to astra Cloud-API and other providers, "
...
# Send transaction to astra Cloud-API
def send_tx_blxr(tx: SignedTransaction, endpoint: str, auth_header: str) -> None:
    ...
    print(f"{datetime.datetime.utcnow()} - Sending transaction to astra Cloud-API. TX Hash: {tx.hash.hex()}")
    result = requests.post(
        endpoint,
    ...
    if "result" not in result_json or "tx_hash" not in result_json["result"]:
        print(f"{datetime.datetime.utcnow()} - Failed to send raw transaction {tx.rawTransaction.hex()} to "
              f"astra Cloud-API, with result {result_json} returned. ")
...
def get_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--blxr-endpoint", type=str, default="http://api.blxrbdn.com:443",
                        help="astra endpoint. Use http://api.blxrbdn.com:443 for Cloud-API.")
    parser.add_argument("--blxr-auth-header", type=str, required=True,
                        help="astra authorization header. Use base64 encoded value of "
                             "account_id:secret_hash for Cloud-API. For more information, see "
                             "https://astra.com/docs/astra-documentation/cloud-api/overview/")
