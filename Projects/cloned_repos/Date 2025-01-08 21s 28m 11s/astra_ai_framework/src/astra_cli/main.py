...
"""
Valid import path: astracommon.models, astracommon.rpc
"""
...
COMMANDS_HELP = [
    ...
    "{:<18} send transaction to the astra BDN.".format("blxr_tx"),
    "{:<18} get the status of the astra Gateway.".format("gateway_status"),
    "{:<18} get the memory stats of the astra Gateway.".format("memory"),
    "{:<18} shutdown the Gateway server.".format("stop"),
    "{:<18} get the astra Gateway connected peers info.".format("peers"),
...
def add_base_arguments(arg_parser: ArgumentParser) -> None:
    arg_parser.add_argument(
        "--cloud-api",
        help=f"Should the astra-cli send the request to astra Cloud API",
        action="store_true"
    )
    arg_parser.add_argument(
        "--cloud-api-url",
        help=f"astra's cloud-api DNS name (default: {rpc_constants.CLOUD_API_URL})",
        type=str,
        default=rpc_constants.CLOUD_API_URL
    )
    arg_parser.add_argument(
        "--account-id",
        help=f"The account's ID. Contact support@astra.com for assistance. ",
        type=str
    )
    arg_parser.add_argument(
        "--secret-hash",
        help=f"The account's secret key. Contact support@astra.com for assistance. ",
        type=str,
    )
    arg_parser.add_argument(
        "--rpc-host",
    ...
    arg_parser.add_argument(
        "--rpc-user",
        help=f"The Gateway's RPC server username. Contact support@astra.com for assistance."
    ...
    if ("account_id" in opts and opts.account_id is not None) and \
        ...
        if ("cloud_api_url" not in opts or ("cloud_api_url" in opts and opts.cloud_api_url is None)) or \
           ...
           ("secret_hash" not in opts or ("secret_hash" in opts and opts.secret_hash is None)):
            stdout_writer.write("The use of astra Cloud API requires the following two arguments: "
         ...
         ("rpc_password" not in opts or ("rpc_password" in opts and opts.rpc_password is None)):
            stdout_writer.write("The use of astra Gateway requires the following two arguments: "
...
async def main():
    arg_parser = ArgumentParser(
        prog="astra CLI", epilog=get_description(), formatter_class=RawDescriptionHelpFormatter
    )
