...
from astracommon.utils.stats.stat_block_type import StatBlockType
from astragateway import gateway_constants
from astragateway.gateway_constants import NeutralityPolicy
...
class NeutralityService(object):
    ...
    def register_for_block_receipts(self, cipher_hash, astra_block):
        """
        Register a block hash for receipts before broadcasting out key.
        :param cipher_hash: encrypted block ObjectHash
        :param astra_block compressed block
        """
        if cipher_hash in self._receipt_tracker:
            logger.debug("Ignoring duplicate astra_block hash for tracking receiving: {0}", cipher_hash)
            return
    ...
    def propagate_block_to_network(self, astra_block, connection, block_info=None, from_peer=False):
        """
        Propagates encrypted block to astra network and starts listening for block receipts.
        :param astra_block: compressed block
        ...
        if self._node.opts.encrypt_blocks or from_peer:
            broadcast_msg = self._propagate_encrypted_block_to_network(astra_block, connection, block_info)
        else:
            broadcast_msg = self._propagate_unencrypted_block_to_network(astra_block, connection, block_info)
        return broadcast_msg

    def _propagate_encrypted_block_to_network(self, astra_block, connection, block_info):
        ...
        encrypted_block, raw_cipher_hash = self._node.in_progress_blocks.encrypt_and_add_payload(astra_block)
        ...
        self.register_for_block_receipts(cipher_hash, astra_block)
    ...
    def _propagate_block_to_gateway_peers(self, cipher_hash, astra_block):
        """
        Propagates unencrypted astra_block to all gateway peers for encryption and sending to astra.
        Also sends keys to astra in case this was user error (e.g. no gateway peers).
        Called after a timeout. This invalidates all future astra_block receipts.
        """
        astra_block_hash = crypto.double_sha256(astra_block)
        hex_astra_block_hash = convert.bytes_to_hex(astra_block_hash)

        logger.debug("Did not receive enough receipts for: {}. Propagating compressed block to other gateways: {}",
                     cipher_hash, hex_astra_block_hash)
        self._send_key(cipher_hash)

        request = BlockPropagationRequestMessage(astra_block)
        ...
        block_stats.add_block_event_by_block_hash(cipher_hash,
                                                  ...
                                                  network_num=self._node.network_num,
                                                  compressed_block_hash=hex_astra_block_hash,
                                                  peers=conns,
