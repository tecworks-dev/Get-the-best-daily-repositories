...
from astracommon.rpc.requests.abstract_blxr_transaction_rpc_request import AbstractBlxrTransactionRpcRequest
from astracommon.rpc.rpc_errors import RpcInvalidParams, RpcAccountIdError, RpcBlocked
...
from astrautils import logging, log_messages as common_log_messages
...
class GatewayBlxrTransactionRpcRequest(AbstractBlxrTransactionRpcRequest["AbstractGatewayNode"]):
    ...
    async def post_process_transaction(
        ...
        try:
            ...
            astra_tx = message_converter.bdn_tx_to_astra_tx(transaction, network_num, transaction_flag, account_id)
        ...
        if self.node.has_active_blockchain_peer():
            blockchain_tx_message = self.node.message_converter.astra_tx_to_tx(astra_tx)
            self.node.broadcast(
                blockchain_tx_message,
                connection_types=(ConnectionType.BLOCKCHAIN_NODE,)
            )

        # All connections outside of this one is a astra server
        ...
        tx_service.set_transaction_contents_by_key(transaction_key, astra_tx.tx_val())
        tx_json = {
            "tx_hash": str(tx_hash),
        }
        if not self.node.account_model.is_account_valid():
            raise RpcAccountIdError(
                ...
                "Please visit https://portal.astra.com to renew your subscription."
            )
