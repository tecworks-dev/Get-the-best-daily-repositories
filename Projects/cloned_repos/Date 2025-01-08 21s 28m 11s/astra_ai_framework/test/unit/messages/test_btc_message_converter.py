...
from astragateway.messages.btc.abstract_btc_message_converter import AbstractBtcMessageConverter
from astragateway.btc_constants import BTC_HDR_COMMON_OFF, BTC_SHA_HASH_LEN
from astragateway.messages.btc.block_btc_message import BlockBtcMessage, BtcMessage
...
from astragateway.testing.mocks import mock_btc_messages
...
class BtcMessageConverterTests(AbstractTestCase):
    ...
    def test_tx_msg_to_btc_tx_msg__success(self):
        ...
        btc_tx_msg = self.btc_message_converter.astra_tx_to_tx(tx_msg)
    ...
    def test_tx_msg_to_btc_tx_msg__type_error(self):
        ...
        self.assertRaises(TypeError, self.btc_message_converter.astra_tx_to_tx, btc_tx_msg)
    ...
    def test_btc_tx_msg_to_tx_msg__success(self):
        ...
        tx_msgs = self.btc_message_converter.tx_to_astra_txs(btc_tx_msg, network_num)
    ...
    def test_btc_tx_msg_to_tx_msg__type_error(self):
        ...
        self.assertRaises(TypeError, self.btc_message_converter.tx_to_astra_txs, tx_msg)

    @multi_setup(True)
    def test_btc_block_to_astra_block_and_back_sids_found(self):
        ...
        astra_block, block_info = self.btc_message_converter.block_to_astra_block(
        ...
        # TODO: if we convert astra block to a class, add some tests here

        parsed_btc_block, block_info, _, _ = self.btc_message_converter.astra_block_to_block(
            astra_block,
            self.tx_service)
    ...
    def test_plain_compression(self):
        ...
        astra_block, astra_block_info = self.btc_message_converter.block_to_astra_block(parsed_block, self.tx_service, True, 0)
        ref_block, block_info, _, _ = self.btc_message_converter.astra_block_to_block(
    ...
    def test_partial_compression(self):
        ...
        for short_id, txn in enumerate(transactions_short):
            astra_tx_hash = btc_common_utils.get_txid(txn)
            self.tx_service.assign_short_id(astra_tx_hash, short_id + 1)
            self.tx_service.set_transaction_contents(astra_tx_hash, txn)
        astra_block, block_info = self.btc_message_converter.block_to_astra_block(parsed_block, self.tx_service, True, 0)
        ref_block, _, unknown_tx_sids, unknown_tx_hashes = self.btc_message_converter.astra_block_to_block(
            astra_block, self.tx_service
        )
    ...
    def test_full_compression(self):
        ...
        for short_id, txn in enumerate(transactions):
            ...
            self.tx_service.set_transaction_contents(astra_tx_hash, txn)
        astra_block, block_info = self.btc_message_converter.block_to_astra_block(parsed_block, self.tx_service, True, 0)
        ref_block, ref_lock_info, unknown_tx_sids, unknown_tx_hashes = self.btc_message_converter.astra_block_to_block(
            astra_block, self.tx_service
        )
    ...
    def test_segwit_partial_compression(self):
        ...
        ref_block, _, _, _ = self.btc_message_converter.astra_block_to_block(
            astra_block, self.tx_service
        )
    ...
    def test_compact_block_full_compression(self):
        ...
        for short_id, txn in enumerate(recovered_block.txns()):
            astra_tx_hash = BtcObjectHash(buf=crypto.double_sha256(txn),
        ...
        result = self.btc_message_converter.compact_block_to_astra_block(
            compact_block, self.tx_service
        )
        self.assertTrue(result.success)
        ref_block, _, _, _ = self.btc_message_converter.astra_block_to_block(
            result.astra_block, self.tx_service
        )
    ...
    def test_compact_block_partial_compression(self):
        ...
        self.assertIsNone(result.astra_block)
        ...
        result = self.btc_message_converter.recovered_compact_block_to_astra_block(result)
        self.assertTrue(result.success)
        ref_block, _, _, _ = self.btc_message_converter.astra_block_to_block(
            result.astra_block, self.tx_service
        )
    ...
    def test_no_compression(self):
        parsed_block = get_sample_block()
        astra_block, block_info = self.btc_message_converter.block_to_astra_block(parsed_block, self.tx_service, False, 0)
        ref_block, _, unknown_tx_sids, unknown_tx_hashes = self.btc_message_converter.astra_block_to_block(
            astra_block, self.tx_service
        )

        self.assertTrue(len(astra_block) >= len(parsed_block.rawbytes()))
