...
import astragateway.messages.eth.eth_message_converter_factory as converter_factory
...
from astragateway.messages.eth.eth_abstract_message_converter import EthAbstractMessageConverter
...
from astracommon.messages.eth.serializers.block import Block
from astragateway.messages.eth.serializers.compact_block import CompactBlock
...
class EthMessageConverterTests(AbstractTestCase):
    ...
    def test_tx_to_astra_tx__success(self):
        txs = [
        ...
        self.validate_tx_to_astra_txs_conversion(tx_msg, txs)

    def test_tx_to_astra_tx__from_bytes_success(self):
        txs = [
            mock_eth_messages.get_dummy_transaction(1),
            mock_eth_messages.get_dummy_transaction(2),
            mock_eth_messages.get_dummy_transaction(3),
        ...
        self.validate_tx_to_astra_txs_conversion(tx_msg_from_bytes, txs)

    def test_tx_to_astra_tx__filter_low_fee(self):
        txs = [
        ...
        results = self.eth_message_converter.tx_to_astra_txs(
        ...
        for i, result in enumerate(results):
            astra_tx_message, tx_hash, tx_contents = result
            self.assertEqual(txs[i].hash(), tx_hash)

    def test_tx_to_astra_tx__from_bytes_single_tx_success(self):
        txs = [
            mock_eth_messages.get_dummy_transaction(1),
        ]
    ...
    def validate_tx_to_astra_txs_conversion(self, tx_msg, txs):
        astra_tx_msgs = self.eth_message_converter.tx_to_astra_txs(tx_msg, self.test_network_num)

        self.assertTrue(astra_tx_msgs)
        self.assertEqual(len(txs), len(astra_tx_msgs))

        for tx, (astra_tx_msg, tx_hash, tx_bytes) in zip(txs, astra_tx_msgs):
            self.assertIsInstance(astra_tx_msg, TxMessage)
            tx_obj = rlp.decode(astra_tx_msg.tx_val().tobytes(), Transaction)
            self.assertEqual(tx, tx_obj)

    def test_astra_tx_to_tx__success(self):
        ...
        astra_tx_message = TxMessage(message_hash=tx_hash, network_num=self.test_network_num, tx_val=tx_bytes)

        tx_message = self.eth_message_converter.astra_tx_to_tx(astra_tx_message)
    ...
    @multi_setup()
    def test_block_to_astra_block__success(self):
        txs = []
        txs_bytes = []
        txs_hashes = []
        short_ids = []
        ...
        astra_block_msg, block_info = self.eth_message_converter.block_to_astra_block(
            internal_new_block_msg, self.tx_service, True, 0
        )
        msg = BroadcastMessage(block_info.block_hash, 5, blob=astra_block_msg)
        ...
        self.assertTrue(astra_block_msg)
        self.assertIsInstance(astra_block_msg, memoryview)

        block_offsets = compact_block_short_ids_serializer.get_astra_block_offsets(astra_block_msg)
        parsed_short_ids, short_ids_len = compact_block_short_ids_serializer.deserialize_short_ids_from_buffer(
            astra_block_msg, block_offsets.short_id_offset
        )
        compact_block = rlp.decode(
            astra_block_msg[block_offsets.block_begin_offset: block_offsets.short_id_offset].tobytes(),
            CompactBlock
        )
    ...
    @multi_setup()
    def test_block_to_astra_block__short_ids_min_time(self):
        txs = []
        txs_bytes = []
        txs_hashes = []
        short_ids = []
    ...
    @multi_setup()
    def test_block_to_astra_block__empty_block_success(self):
    ...
    @multi_setup()
    def test_astra_block_to_block__success(self):
        tx_count = 1500
        txs = []
        short_txs = []
        short_ids = []
        ...
        astra_block_msg = BroadcastMessage(compact_block_hash, self.test_network_num, is_encrypted=True,
                                        blob=compact_block_msg_bytes)

        block_msg, block_info, unknown_tx_sids, unknown_tx_hashes = self.eth_message_converter.astra_block_to_block(
            astra_block_msg.blob(), self.tx_service)
    ...
    @multi_setup()
    def test_astra_block_to_block__full_txs_success(self):
        tx_count = 150
        txs = []
        short_txs = []
        short_ids = []
    ...
    @multi_setup()
    def test_block_to_astra_block_then_astra_block_to_block__success(self):
        txs = []
        txs_bytes = []
        txs_hashes = []
        short_ids = []
        ...
        self.assertIsNotNone(astra_block_msg)

        self.assertEqual(len(txs), block_info.txn_count)
        self.assertEqual(convert.bytes_to_hex(block.header.prev_hash), block_info.prev_block_hash)

        converted_block_msg, _, _, _ = self.eth_message_converter.astra_block_to_block(astra_block_msg, self.tx_service)
    ...
    @multi_setup()
    def test_block_to_astra_block__no_compressed_block(self):
        txs = []
        txs_bytes = []
        txs_hashes = []
        short_ids = []
        ...
        self.assertTrue(len(astra_block_msg) >= len(internal_new_block_msg.rawbytes()))
