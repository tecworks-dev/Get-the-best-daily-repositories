...
export class AppModuleBuilder {
  private readonly modules: BeeSyncModule[] = [];
  constructor(private readonly config: Config) {}

  use(...modules: BeeSyncModule[]): this {
    modules.forEach(m => {
      ...
      if (requirements?.length) {
        ...
        if (nonMetRequirements.length) {
          ...
          new Logger(name).warn(
            ...
            ...nonMetRequirements.map(config => `  BeeSync.${config}`)
          );
          return;
        }
      }
...
function buildAppModule() {
  const factor = new AppModuleBuilder(BeeSync);
  ...
  // plugin modules
  BeeSync.plugins.enabled.forEach(name => {
