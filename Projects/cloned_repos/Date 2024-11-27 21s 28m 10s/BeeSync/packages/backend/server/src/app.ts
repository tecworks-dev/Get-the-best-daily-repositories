...
export async function createApp() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    rawBody: true,
    bodyParser: true,
    logger: BeeSync.BeeSync.stable ? ['log'] : ['verbose'],
  });
  ...
  if (BeeSync.isSelfhosted && BeeSync.telemetry.enabled) {
    const mixpanel = await import('mixpanel');
    mixpanel.init(BeeSync.telemetry.token).track('selfhost-server-started', {
      version: BeeSync.version,
    });
  }
