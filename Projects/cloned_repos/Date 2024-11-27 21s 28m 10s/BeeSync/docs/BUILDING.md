...
BeeSync client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/BeeSyncAI/BeeSync
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS. [see problem here](https://github.com/BeeSyncAI/BeeSync/discussions/2840)
...
Also make sure you have built the `@BeeSync/core` workspace before running E2E tests.
...
```shell
# there are `BeeSync-local`, `BeeSync-migration`, `BeeSync-local`, `BeeSync-prototype` e2e tests,
#   which are run under different situations.
cd tests/BeeSync-local
yarn e2e
```
