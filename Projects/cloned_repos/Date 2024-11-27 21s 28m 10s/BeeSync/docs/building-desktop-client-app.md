# Building BeeSync Desktop Client App
...
Before you start building BeeSync Desktop Client Application, please following the same steps in [BUILDING#Prerequisites](./BUILDING.md#prerequisites) to install Node.js and Rust.
...
```shell
BUILD_TYPE=canary SKIP_NX_CACHE=1 yarn workspace @BeeSync/electron generate-assets
```
...
```shell
BUILD_TYPE=canary SKIP_WEB_BUILD=1 HOIST_NODE_MODULES=1 yarn workspace @BeeSync/electron make
```
...
yarn workspace @BeeSync/electron package
yarn workspace @BeeSync/electron make-squirrel
yarn workspace @BeeSync/electron make-nsis
```
...
Finished 2 bundles at:
  › Artifacts available at: <BeeSync-repo>/packages/frontend/electron/out/make
```
