...
class NeutralityServiceTest(AbstractTestCase):
    ...
    @patch("astragateway.gateway_constants.NEUTRALITY_POLICY", NeutralityPolicy.RELEASE_IMMEDIATELY)
    ...
    @patch("astragateway.gateway_constants.NEUTRALITY_EXPECTED_RECEIPT_COUNT", 2)
    @patch("astragateway.gateway_constants.NEUTRALITY_POLICY", NeutralityPolicy.RECEIPT_COUNT)
    ...
    @patch("astragateway.gateway_constants.NEUTRALITY_EXPECTED_RECEIPT_PERCENT", 50)
    @patch("astragateway.gateway_constants.NEUTRALITY_POLICY", NeutralityPolicy.RECEIPT_PERCENT)
    ...
    @patch("astragateway.gateway_constants.NEUTRALITY_EXPECTED_RECEIPT_COUNT", 4)
    @patch("astragateway.gateway_constants.NEUTRALITY_EXPECTED_RECEIPT_PERCENT", 50)
    @patch("astragateway.gateway_constants.NEUTRALITY_POLICY", NeutralityPolicy.RECEIPT_COUNT_AND_PERCENT)
    def test_release_key_after_enough_receipts_percent(self):
        self._add_num_gateway_connections(6)
        self.node.in_progress_blocks.get_encryption_key = MagicMock(return_value=self.KEY_HASH)
    ...
    def test_propagate_block_to_network_unencrypted_block(self):
        ...
        self.assertTrue(all(ConnectionType.RELAY_BLOCK in connection_type for connection_type in connection_types))
    ...
    def _assert_broadcast_key(self):
        ...
        self.assertTrue(connection_types[0] in ConnectionType.RELAY_ALL)
