...
from astragateway.abstract_message_converter import finalize_block_bytes
from astragateway.messages.btc.abstract_btc_message_converter import AbstractBtcMessageConverter, get_block_info, \
...
from astragateway.messages.btc import btc_messages_util
...
def parse_astra_block_header(
        astra_block: memoryview, block_pieces: Deque[Union[bytearray, memoryview]]
) -> BlockHeaderInfo:
    ...
    # Add header piece
    txn_count, txn_count_size = btc_common_utils.btc_varint_to_int(astra_block, block_header_size)
    offset += txn_count_size
    block_pieces.append(astra_block[block_offsets.block_begin_offset:offset])
    return BlockHeaderInfo(block_offsets, short_ids, short_ids_len, block_hash, offset, txn_count)


def parse_astra_block_transactions(
    ...
    while offset < block_offsets.short_id_offset:
        if astra_block[offset] == btc_constants.BTC_SHORT_ID_INDICATOR:
            try:
            ...
            tx_size = btc_messages_util.get_next_tx_size(astra_block, offset)
...
class BtcNormalMessageConverter(AbstractBtcMessageConverter):
    ...
    def block_to_astra_block(
        ...
        """
        Compresses a Bitcoin block's transactions and packs it into a astra block.
        """
    ...
    def compact_block_to_astra_block(
        ...
        return self._recovered_compact_block_to_astra_block(result, recovered_item)
    ...
    def _recovered_compact_block_to_astra_block(
        ...
        astra_block, compression_block_info = self.block_to_astra_block(
        ...
        return CompactBlockCompressionResult(True, block_info, astra_block, None, [], [])
