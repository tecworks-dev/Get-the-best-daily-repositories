...
from astracommon.utils import convert, memory_utils
...
from astragateway.messages.ont import abstract_ont_message_converter
from astragateway.messages.ont import ont_normal_consensus_message_converter
...
class OntExtensionConsensusMessageConverter(AbstractOntMessageConverter):
    ...
    def block_to_astra_block(
        ...
        tsk.init(
            tpe.InputBytes(block_msg.buf), extension_tx_service.proxy, enable_block_compression, min_tx_age_seconds
        )
        try:
    ...
    def astra_block_to_block(
            self, astra_block_msg: memoryview, tx_service: ExtensionTransactionService
        ...
        try:
            ...
            header_info = ont_normal_consensus_message_converter.parse_astra_block_header(astra_block_msg, deque())
