...
from astrautils.encoding.json_encoder import EnhancedJSONEncoder
...
from astracommon.utils import convert, crypto
...
from astragateway import ont_constants, log_messages
from astragateway.abstract_message_converter import BlockDecompressionResult, finalize_block_bytes
from astragateway.messages.ont import ont_messages_util
from astragateway.messages.ont.abstract_ont_message_converter import AbstractOntMessageConverter, get_block_info
from astragateway.messages.ont.consensus_ont_message import OntConsensusMessage, ConsensusMsgPayload
from astragateway.utils.block_header_info import BlockHeaderInfo
from astragateway.utils.block_info import BlockInfo
from astragateway.utils.errors import message_conversion_error

logger = logging.get_logger(__name__)


def parse_astra_block_header(astra_block: memoryview, block_pieces: Deque[Union[bytearray, memoryview]]) -> \
        BlockHeaderInfo:
    block_offsets = compact_block_short_ids_serializer.get_astra_block_offsets(astra_block)
    ...
    block_hash = OntObjectHash(binary=astra_block[block_offsets.block_begin_offset + 1:
    ...
    txn_count, = struct.unpack_from("<L", astra_block, offset)
    offset += ont_constants.ONT_INT_LEN
    payload_tail_len, = struct.unpack_from("<L", astra_block, offset)
    offset += ont_constants.ONT_INT_LEN + payload_tail_len
    owner_and_signature_len, = struct.unpack_from("<L", astra_block, offset)
    offset += ont_constants.ONT_INT_LEN + owner_and_signature_len
    consensus_payload_header_len, = struct.unpack_from("<L", astra_block, offset)
    offset += ont_constants.ONT_INT_LEN
    block_pieces.append(astra_block[offset: offset + consensus_payload_header_len])
    offset += consensus_payload_header_len
    block_pieces.append(astra_block[offset: offset + ont_constants.ONT_CHAR_LEN])
    offset += ont_constants.ONT_CHAR_LEN
    block_pieces.append(astra_block[offset: offset + ont_constants.ONT_INT_LEN])
    offset += ont_constants.ONT_INT_LEN
    block_start_len_and_txn_header_total_len, = struct.unpack_from("<L", astra_block, offset)
    offset += ont_constants.ONT_INT_LEN
    block_pieces.append(astra_block[offset: offset + block_start_len_and_txn_header_total_len])
...
def parse_astra_block_transactions_and_msg_tail(block_hash: Sha256Hash, astra_block: memoryview, offset: int,
    ...
    while offset < block_offsets.short_id_offset:
        if astra_block[offset] == ont_constants.ONT_SHORT_ID_INDICATOR:
            try:
            ...
            tx_size = ont_messages_util.get_next_tx_size(astra_block, offset)
            tx = astra_block[offset:offset + tx_size]
            offset += tx_size
    ...
    payload_tail_len, = struct.unpack_from("<L", astra_block, offset)
    offset += ont_constants.ONT_INT_LEN
    block_pieces.append(astra_block[offset: offset + payload_tail_len])
    offset += payload_tail_len
    owner_and_signature_len, = struct.unpack_from("<L", astra_block, offset)
    offset += ont_constants.ONT_INT_LEN
    block_pieces.append(astra_block[offset: offset + owner_and_signature_len])
...
class OntNormalConsensusMessageConverter(AbstractOntMessageConverter):
    ...
    def block_to_astra_block(
        ...
        """
        Pack an Ontology consensus message's transactions into a astra block.
        """
        ...
        # Prepend owner and signature, consensus payload tail, tx count and block hash to astra_block
        ...
        astra_block_hash = convert.bytes_to_hex(crypto.double_sha256(block))
    ...
    def astra_block_to_block(self, astra_block_msg: memoryview, tx_service: TransactionService) -> BlockDecompressionResult:
        """
        Uncompresses a astra_block from a broadcast astra_block message and converts to a raw Ontology astra_block.
        ...
        block_pieces = deque()
        header_info = parse_astra_block_header(astra_block_msg, block_pieces)
        unknown_tx_sids, unknown_tx_hashes, offset = parse_astra_block_transactions_and_msg_tail(
        ...
        if not unknown_tx_sids and not unknown_tx_hashes:
            ...
            logger.debug("Successfully parsed astra_block broadcast message. {} transactions in astra_block", total_tx_count)
        else:
