...
from astragateway.abstract_message_converter import BlockDecompressionResult
...
def parse_astra_block_header(astra_block: memoryview, block_pieces: Deque[Union[bytearray, memoryview]]) -> \
    ...
    reconstructed_block_message = BlockOntMessage(
        buf=astra_block[block_offsets.block_begin_offset + ont_constants.ONT_HASH_LEN + 1:])
    ...
    # Add header piece
    block_pieces.append(astra_block[block_offsets.block_begin_offset + ont_constants.ONT_HASH_LEN + 1:offset])
    return BlockHeaderInfo(block_offsets, short_ids, short_ids_len, block_hash, offset, txn_count)


def parse_astra_block_transactions(block_hash: Sha256Hash, astra_block: memoryview, offset: int, short_ids: List[int],
    ...
    merkle_root = astra_block[block_offsets.block_begin_offset + 1:
...
class OntNormalMessageConverter(AbstractOntMessageConverter):
    ...
    def block_to_astra_block(
        ...
        """
        Pack an Ontology block's transactions into a astra block.
        """
    ...
    def astra_block_to_block(self, astra_block_msg: memoryview, tx_service: TransactionService) -> BlockDecompressionResult:
        ...
        if not isinstance(astra_block_msg, memoryview):
            astra_block_msg = memoryview(astra_block_msg)
        ...
        unknown_tx_sids, unknown_tx_hashes, offset = parse_astra_block_transactions(
