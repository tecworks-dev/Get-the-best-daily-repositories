...
class OntBlockProcessingService(BlockProcessingService):
    ...
    def __init__(self, node):
        super().__init__(node)
        self._node = cast("astragateway.connections.ont.ont_gateway_node.OntGatewayNode", node)
        ...
        if self._node.has_active_blockchain_peer() or self._node.remote_node_conn:
            try:
                block_message, block_info, unknown_sids, unknown_hashes = \
                    self._node.consensus_message_converter.astra_block_to_block(astra_block, transaction_service)
