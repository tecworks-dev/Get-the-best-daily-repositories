...
from astracommon.rpc.rpc_errors import RpcInvalidParams, RpcInternalError
        ...
        if rpc_constants.BLOCKCHAIN_PEER_PARAMS_KEY in params:
            ...
            if blockchain_protocol is not None:
                ...
                raise RpcInternalError(
                    self.request_id,
                    "Could not process request to add/remove blockchain peer. Please contact astra support."
                )
        else:
            raise RpcInvalidParams(
                self.request_id,
