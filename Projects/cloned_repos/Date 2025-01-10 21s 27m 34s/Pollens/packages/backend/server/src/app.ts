...
export async function createApp() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    rawBody: true,
    bodyParser: true,
    logger: Pollens.Pollens.stable ? ['log'] : ['verbose'],
  });
  ...
  if (Pollens.isSelfhosted && Pollens.telemetry.enabled) {
    const mixpanel = await import('mixpanel');
    mixpanel.init(Pollens.telemetry.token).track('selfhost-server-started', {
      version: Pollens.version,
    });
  }
