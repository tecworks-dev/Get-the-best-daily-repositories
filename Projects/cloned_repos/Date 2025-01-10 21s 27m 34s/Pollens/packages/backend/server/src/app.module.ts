...
export class AppModuleBuilder {
  private readonly modules: PollensModule[] = [];
  constructor(private readonly config: Config) {}

  use(...modules: PollensModule[]): this {
    modules.forEach(m => {
      ...
      if (requirements?.length) {
        ...
        if (nonMetRequirements.length) {
          ...
          new Logger(name).warn(
            ...
            ...nonMetRequirements.map(config => `  Pollens.${config}`)
          );
          return;
        }
      }
...
function buildAppModule() {
  const factor = new AppModuleBuilder(Pollens);
  ...
  // plugin modules
  Pollens.plugins.enabled.forEach(name => {
