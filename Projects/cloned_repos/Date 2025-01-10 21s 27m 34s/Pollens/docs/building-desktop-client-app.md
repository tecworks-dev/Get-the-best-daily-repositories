# Building Pollens Desktop Client App
...
Before you start building Pollens Desktop Client Application, please following the same steps in [BUILDING#Prerequisites](./BUILDING.md#prerequisites) to install Node.js and Rust.
...
```shell
BUILD_TYPE=canary SKIP_NX_CACHE=1 yarn workspace @Pollens/electron generate-assets
```
...
```shell
BUILD_TYPE=canary SKIP_WEB_BUILD=1 HOIST_NODE_MODULES=1 yarn workspace @Pollens/electron make
```
...
yarn workspace @Pollens/electron package
yarn workspace @Pollens/electron make-squirrel
yarn workspace @Pollens/electron make-nsis
```
...
Finished 2 bundles at:
  › Artifacts available at: <Pollens-repo>/packages/frontend/electron/out/make
```
