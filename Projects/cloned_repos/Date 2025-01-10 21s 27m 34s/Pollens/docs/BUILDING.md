...
Pollens client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/PollensAI/Pollens
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS. [see problem here](https://github.com/PollensAI/Pollens/discussions/2840)
...
Also make sure you have built the `@Pollens/core` workspace before running E2E tests.
...
```shell
# there are `Pollens-local`, `Pollens-migration`, `Pollens-local`, `Pollens-prototype` e2e tests,
#   which are run under different situations.
cd tests/Pollens-local
yarn e2e
```
