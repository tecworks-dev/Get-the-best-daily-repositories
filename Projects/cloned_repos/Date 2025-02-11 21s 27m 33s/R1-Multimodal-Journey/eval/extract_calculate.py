...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_file', type=str, default='mathvista.json')
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        futures = [executor.submit(extract_answer, results[sample_id][label], results[sample_id]) for sample_id in
                   test_pids]

        for future in as_completed(futures):
            extraction, pid = future.result()
    ...
    with open(result_file.replace('.json', f'_score.json'), 'w') as f:
        json.dump(json.loads(scores.to_json(orient='records')), f, indent=4, ensure_ascii=False)
