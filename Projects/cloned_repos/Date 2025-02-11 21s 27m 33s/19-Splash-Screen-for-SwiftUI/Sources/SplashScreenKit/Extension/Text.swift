...
public struct AppearanceEffectRenderer: TextRenderer, Animatable {
    ...
    public var elementDuration: TimeInterval
    ...
    public init(elapsedTime: TimeInterval, elementDuration: Double = 0.4, totalDuration: TimeInterval) {
    ...
    public func draw(_ slice: Text.Layout.RunSlice, at time: TimeInterval, in context: inout GraphicsContext) {
        ...
        let translationY = spring.value(
            fromValue: slice.typographicBounds.descent,         // Using -fromValue, the text translate in opposite way
            toValue: 0,
            initialVelocity: 0,
            time: time)
    ...
    public func elementDelay(count: Int) -> TimeInterval {
