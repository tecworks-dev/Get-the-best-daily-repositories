...
# Function to update a specific conditional access policy
function Update-ConditionalAccessPolicy {
    ...
    $response = Invoke-MgGraphRequest -Uri $uri -Method PATCH -Body ($body | ConvertTo-Json -Depth 10)
    return $response
}
...
function Remove-OrphanedUserFromPolicy {
    ...
    if ($isUpdated) {
        try {
            $updatedPolicy = Update-ConditionalAccessPolicy -policyId $policyId -conditions $policy.conditions
            Write-Host "Updated policy: $($policy.displayName) (ID: $policyId)" -ForegroundColor Green

            # Re-fetch the policy to confirm the changes
            $confirmedPolicy = Get-ConditionalAccessPolicyById -policyId $policyId
            if ($confirmedPolicy.conditions.users.includeUsers -contains $orphanedUserId -or $confirmedPolicy.conditions.users.excludeUsers -contains $orphanedUserId) {
