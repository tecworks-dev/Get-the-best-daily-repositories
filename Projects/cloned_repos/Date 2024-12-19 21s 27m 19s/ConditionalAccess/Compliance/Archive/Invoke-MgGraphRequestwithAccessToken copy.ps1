...
$accessToken = Get-MsGraphAccessToken -tenantId $tenantId -clientId $clientId -certThumbprint $certThumbprint
...
# Write-EnhancedLog -Message "Raw Tenant Response: $($tenantResponse.Content)" -Level "INFO" -ForegroundColor ([ConsoleColor]::White)

# if ($tenantResponse.Content) {
#     $organization = ($tenantResponse.Content | ConvertFrom-Json).value[0]
...
# $responseContent = $tenantResponse.Content
# Write-EnhancedLog -Message "Raw Tenant Response Content: $responseContent" -Level "INFO" -ForegroundColor ([ConsoleColor]::White)

# $parsedResponse = $responseContent | ConvertFrom-Json
# Write-EnhancedLog -Message "Parsed Response: $($parsedResponse | Out-String)" -Level "INFO" -ForegroundColor ([ConsoleColor]::White)
...
# Assuming you have already authenticated using Connect-MgGraph

# Retrieve the access token from the current session
# $accessToken = (Get-MgProfile -ErrorAction Stop).AuthToken
