...
let rec transl_expr ~name_hint ~mtype_defs ~addr_tbl ~type_defs ~object_methods
  ...
  let go x =
    transl_expr ~name_hint ~mtype_defs ~addr_tbl ~type_defs ~object_methods x
      [@@inline]
  in
  let bind (rhs : Mcore.expr) cont : Clam.lambda =
    match rhs with
    | Cexpr_var { id; ty; _ } ->
  ...
  let expr : Clam.lambda =
    ...
    | Cexpr_prim { prim; args; ty } -> (
        ...
        | Parray_make ->
            ...
            let arr_tid =
              ...
              | Some (Record { fields = { field_type = arr_type; _ } :: _ }) ->
                  transl_type_as_named_exn arr_type
              | _ -> assert false
            in
            ...
            Clam.Lallocate
              {
                ...
                fields =
                  [
                    Clam.Lmake_array
                      { kind = EverySingleElem; tid = arr_tid; elems = args };
                    Clam.Lconst
                      (C_int
                         { v = Int32.of_int (List.length args); repr = None });
                  ];
              }
        ...
        | Pfixedarray_get_item { kind } -> (
            ...
            if arr_type = T_bytes then
              Lprim { fn = Pgetbytesitem; args = Lst.map args go }
            else
              let need_non_null_cast =
                let elem = Mtype.get_fixedarray_elem_exn arr_type in
                Mtype.is_uninit elem
              in
        ...
        | Penum_field { index; tag } -> (
            ...
            in
            match[@warning "-fragile-match"] List.map go args with
            | obj :: [] ->
        ...
        | Pmake_value_or_error { tag } ->
            ...
            Lallocate
              { kind = Enum { tag }; tid = t; fields = List.map go args }
            ...
            let prim : Clam.intrinsic option =
              ...
              | Some
                  (Pintrinsic
                    ...
                    | Iter_flat_map | Iter_repeat | Iter_filter | Iter_concat
                    | Array_length | Array_get | Array_unsafe_get | Array_set
                    | Array_unsafe_set )) ->
                  None
              | Some _ -> assert false
              | None -> None
    ...
    | Cexpr_tuple { exprs; ty } ->
        ...
        Lallocate { kind = Tuple; tid = t; fields = List.map go exprs }
    ...
    | Cexpr_array { exprs; ty } ->
        ...
        Lmake_array { kind = EverySingleElem; tid; elems = List.map go exprs }
    ...
    | Cexpr_if { cond; ifso; ifnot; ty } -> (
        ...
        | None -> Lif { pred; ifso; ifnot = clam_unit; type_ = I32_Unit })
    ...
    | Cexpr_handle_error { obj; handle_kind; ty } ->
        ...
        bind obj (fun obj_id ->
            ...
            let action_ok : Clam.lambda =
              ...
              Lget_field
                {
                  kind = Enum { tag = ok_tag };
                  obj =
                    Lcast { expr = obj; target_type = Ref { tid = constr_tid } };
                  index = 0;
                  tid = constr_tid;
                }
            in
            ...
            let default : Clam.lambda =
              ...
              | Joinapply err_join ->
                  ...
                  in
                  Ljoinapply
                    { name = Join.of_core_ident err_join; args = [ err_value ] }
    ...
    | Cexpr_switch_constr { obj; cases; default; ty } ->
        ...
        bind obj (fun obj_id ->
            let obj : Clam.lambda = Lvar { var = obj_id } in
            let transl_action tag binder body : Clam.lambda =
              match binder with
              | None -> go body
              | Some binder ->
    ...
    | Cexpr_continue { args; label; ty = _ } ->
        Lcontinue { args = List.map go args; label }
...
and closure_of_fn ~mtype_defs ~addr_tbl ~type_defs ~object_methods ~name_hint
    ...
    ~(abs_closure_tid : Tid.t) (fn : Mcore.fn) : Clam.closure * Ltype.t =
  let exclude =
    match self with
  ...
  | _ ->
      ...
      let body =
        match self with
        | Some self -> Clam.Llet { name = self; e = env; body = body_with_cast }
        | None -> body_with_cast
      in
      let fn : Clam.fn = { params = env_id :: params; body; return_type_ } in
      new_top { binder = address; fn_kind_ = Clam.Top_private; fn; tid = None };
...
and well_known_closure_of_fn ~mtype_defs ~addr_tbl ~type_defs ~object_methods
    ...
    (fn : Mcore.fn) : Clam.lambda * Ident.t =
  let params = transl_params ~mtype_defs ~type_defs fn.params in
  let return_type_ =
    [
      Transl_mtype.mtype_to_ltype ~mtype_defs ~type_defs
...
let make_top_fn_item ~mtype_defs ~addr_tbl ~type_defs ~object_methods
    ...
    ~export_info_ : Clam.top_func_item =
  let fn_kind_ =
...
let make_top_closure_item (addr : Fn_address.t) (params : Ident.t list)
    (return_type_ : Ltype.t list) ~(closure_ty : Ltype.t) : Clam.top_func_item =
...
let make_object_wrapper ~mtype_defs ~type_defs ~addr_tbl ~object_methods
    ~abstract_obj_tid ~concrete_obj_tid ~self_lty ~trait ~method_index
...
let sequence (a : Clam.lambda) (b : Clam.lambda) ~expr1_type_ =
  if Basic_prelude.phys_equal (Clam_util.no_located b) clam_unit then a
  else if Basic_prelude.phys_equal (Clam_util.no_located a) clam_unit then b
...
let transl_top_item ~(mtype_defs : Mtype.defs) ~(addr_tbl : Addr_table.t)
  ...
  | Ctop_stub { binder; func_stubs; params_ty; return_ty; export_info_; loc_ }
    -> (
      ...
      | Toplevel { addr; params; return; _ } ->
          let args =
            Lst.map2 params params_ty (fun id ty : Clam.lambda ->
          ...
          in
          let ret = Option.value return_ty ~default:I32_Unit in
          let fn_item =
    ...
    Addr_table.fold addr_tbl (globals, start, !binds_init)
      (fun _fn_binder fn_info (globals, start, fns) ->
        ...
        | Toplevel { name_as_closure = Some name; params; return; addr } -> (
            ...
            | Ref { tid } ->
                let fn_closure =
                  Clam.Lclosure { captures = []; address = Normal address; tid }
                in
                ( (name, None) :: globals,
