...
install_validator() {
  ...
  ansible-playbook --connection=local --inventory ./inventory/$inventory --limit localhost playbooks/pb_install_validator.yaml --extra-vars "/etc/sv_manager/sv_manager.conf"

  echo "### Cleaning up Ansible installation ###"
  $pkg_manager remove ansible --yes

  if [ "$inventory" = "mainnet.yaml" ]; then
    echo "### Solana is ready. Start manually using 'systemctl start solana-validator'. ###"
  fi
