...
// DockAreaWidget.cpp
//
// Copyright (c) 2023 N-GINN LLC. All rights reserved.
...
private:
	...
	CDockAreaWidget::eDockAreaTabsPosition m_TabsPosition = CDockAreaWidget::North;
...
public:
	...
	void setTabsPosition(CDockAreaWidget::eDockAreaTabsPosition TabsPosition)
	{
		if (TabsPosition != CDockAreaWidget::North && TabsPosition != CDockAreaWidget::South)
		{
			assert(!"Not implemented tabs position");
		}
		
		if (m_TabsPosition == TabsPosition)
		{
			return;
		}

		if (m_ParentLayout->count() == 2)
		{
			// Swap header and content. Note that we support only Nord and South at this moment.
			m_ParentLayout->insertItem(0, m_ParentLayout->takeAt(1));
		}

		m_TabsPosition = TabsPosition;
	}

	CDockAreaWidget::eDockAreaTabsPosition tabsPosition() const
	{
		return m_TabsPosition;
	}
	...
	void removeWidget(QWidget* Widget)
	{
		...
		{
			auto LayoutItem = takeItemFromParentLayout();
			if (LayoutItem)
			{
...
private:
	QLayoutItem* takeItemFromParentLayout()
	{
		if (m_TabsPosition == CDockAreaWidget::North)
		{
			return m_ParentLayout->takeAt(1);
		}
		else if (m_TabsPosition == CDockAreaWidget::South)
		{
			return m_ParentLayout->count() == 1
				? m_ParentLayout->takeAt(1)
				: m_ParentLayout->takeAt(0);
		}
		else
		{
			assert(!"Not implemented tab position");
		}

		return nullptr;
	}

	void addWidgetInternal(QWidget* widget)
	{
		if (m_TabsPosition == CDockAreaWidget::North)
		{
			return m_ParentLayout->addWidget(widget);
		}
		else if (m_TabsPosition == CDockAreaWidget::South)
		{
			m_ParentLayout->insertWidget(0, widget);
		}
		else
		{
			assert(!"Not implemented tab position");
...
void CDockAreaWidget::updateTitleBarVisibility()
{
    ...
    if (!CDockManager::testConfigFlag(CDockManager::AlwaysShowTabs))
    {
		...
		{
			CDockWidget* widget = dockWidget(0);
			if (widget->features().testFlag(CDockWidget::DockWidgetFeature::DockWidgetAloneHasNoTitleBar))
			{
				Hidden = true;
			}
		}
...
void CDockAreaWidget::saveState(QXmlStreamWriter& s) const
{
	...
	s.writeAttribute("TabsPosition", QString::number(d->ContentsLayout->tabsPosition()));
		...
		bool Testing, CDockContainerWidget* Container)
{
	...
	if (!Testing)
	{
		...
		const auto TabsPositionAttribute = s.attributes().value("TabsPosition");
		if (!TabsPositionAttribute.isEmpty())
		{
			DockArea->setTabsPosition((CDockAreaWidget::eDockAreaTabsPosition)TabsPositionAttribute.toInt());
		}
	}
...
void CDockAreaWidget::setTabsPosition(eDockAreaTabsPosition tabsPosition)
{
	d->ContentsLayout->setTabsPosition(tabsPosition);
}
