...
@torch.no_grad()
def eval(input_folder, target_folder, front_view_idx, device="cuda"):
    src_img = os.path.join(target_folder, f"{front_view_idx:05d}.png")
    if not os.path.exists(src_img):
        return -1

    head_img = crop_face_image(src_img)
    input_imgs = get_image_paths_current_dir(input_folder)
    if "visualization" in input_imgs[-1]:
    ...
    if len(face_id_loss_list) > 0:
        ...
        return min(face_id_loss_list)  # return max similarity view.
    else:
        return -1
...
if __name__ == "__main__":
    ...
    save_folder = os.path.join("./exps/metrics", "psnr_results", *target_key)
    ...
    for item_ in tqdm(items):

        try:
            item, front_view_idx = item_
        ...
        if os.path.exists(input_item_folder) and os.path.exists(target_item_folder):
            ...
            fs_ = eval(input_item_folder, target_item_folder, front_view_idx)
