...
func TestMain(m *testing.M) {
	...
	testCases = []testCase{
		{
			testingName:    "Band Yoda Package",
			endpoints:      common.Endpoints{APIs: []string{os.Getenv("TEST_BAND_ENDPOINT")}},
			chainName:      "band",
			protocolType:   "cosmos",
			expectedResult: 1,
		},
...
func TestYodaPackageInVMSMode(t *testing.T) {
	...
	retryPoint:
		for _, metricName := range testMetrics {
			...
			if string(body) == "" && retryCount == 0 {
				t.Logf("you should sleep more for cvms")
				time.Sleep(3 * time.Second)
				retryCount++
				goto retryPoint
			}
...
func TestYodaPackageInSoloMode(t *testing.T) {
	// additional setup
