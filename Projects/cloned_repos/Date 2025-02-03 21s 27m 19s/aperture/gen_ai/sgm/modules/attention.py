...
class CrossAttention(nn.Module):
    def __init__(
        ...
        # this is 1024 for svd.yaml. why does this determine if the attn is self or cross?
        #
        # Ans:  If context_dim is not provided (or is set to the same size as query_dim), the model 
        #       assumes  K  and  V  are derived from the same source as  Q .
    ...
    def forward(
        ...
        # so what actually is the 'query' and 'key' here?
        # is it the noisy image and the input image?
        #
        # here, is the context the vae conditioning of an image?
        q = self.to_q(x)
        ...
        if self._is_attn2:
            ## old
            # Suppose: q, k, v = [b, h, n, d]
            sim = torch.einsum("b h i d, b h j d -> b h i j", q, k) * self.scale  # [b, h, i, j]
            del q, k
            ...
            # This is only for temporal attention
            # ---
            # ok well now I know that the sim shape sim shape torch.Size([18432, 5, 14, 1]) is actually
            # ([2*9216, 5, 14, 1]). and the 9216 comes from the downsampled input of 1024x576->128x72. 
            #
            # for spatial, shape is torch.Size([28, 5, 9216, 1])
            ...
            out = torch.einsum("b h i j, b h j d -> b h i d", sim, v)  # [b, h, i, d]
