...
static PPC_INSTRUCTION s_TrampolineBuffer[1024 / sizeof(PPC_INSTRUCTION)] = { 0 };
static PPC_INSTRUCTION s_EmulationBuffer[1024 / sizeof(PPC_INSTRUCTION)] = { 0 };
...
static size_t PPCHook_EmitAbsoluteBranchImpl(PPPC_INSTRUCTION Destination, const PPPC_INSTRUCTION Target, bool Call, ULONG BranchOptions, ULONG ConditionRegister) {
...
static size_t PPCHook_EmitAbsoluteBranch(PPPC_INSTRUCTION Destination, const PPPC_INSTRUCTION Target) {
...
static size_t PPCHook_RelocateBranch(PPPC_INSTRUCTION Destination, PPPC_INSTRUCTION Source) {
	if (Source->Iform_AA) {
...
static size_t PPCHook_RelocateEmulation(PPPC_INSTRUCTION Destination, PPPC_INSTRUCTION Source) {
	if (Destination == NULL) return 2;
	PPC_INSTRUCTION Insn = { 0 };
	Insn.Primary_Op = EMU_OP;
	Insn.Emu_Offset = s_EmulationSize;
	ULONG TableOffset = Source->Emu_Offset;
	TableOffset *= sizeof(ULONG);
	PPPC_INSTRUCTION Real = (PPPC_INSTRUCTION)((ULONG)Source + TableOffset);
...
size_t PPCHook_HookWithCave(PVOID* FunctionPointer, PVOID HookLocation, PPPC_INSTRUCTION TrampolineCave) {
	s_EmulationSize = 0;
	// Get the length of the hook (in instructions)
	PPPC_INSTRUCTION Function = (PPPC_INSTRUCTION)*FunctionPointer;
	...
	if (InsnCount == 1) BranchToCaveInOne = false;
		...
		HookSize = PPCHook_EmitAbsoluteBranch(TrampolineCave, (PPPC_INSTRUCTION)HookLocation);
	...
	if (TrampolineCave != NULL) {
		*FunctionPointer = &TrampolineCave[RelocationStart];
		...
		PPPC_INSTRUCTION HookLocationInsn = (PPPC_INSTRUCTION)HookLocation;
