...
#else
static KPCR* __builtin_get_sprg1(void) {
	ULONG PcrValue;
	asm volatile("mfsprg %0, 1\n" : "=r" (PcrValue));
	return (KPCR*)PcrValue;
}
...
// BUGBUG: except it did, somewhere between 1314 and 1381 lol
...
typedef struct _KINTERRUPT_OLD {
    CSHORT Type;
    CSHORT Size;
    LIST_ENTRY InterruptListEntry;
    PKSERVICE_ROUTINE ServiceRoutine;
    ...
    BOOLEAN ShareVector;
    ULONG DispatchCode[DISPATCH_LENGTH];
} KINTERRUPT_OLD, *PKINTERRUPT_OLD;
...
// ObfReferenceObject was added in NT4 so:
// specifying KernelMode does bypass the check against ObjectType so pass NULL there
// DesiredAccess here does not mean anything and is not used
#define ObReferenceObject(Object) ObReferenceObjectByPointer(Object, 0, NULL, KernelMode)
