...
class EvaluationIndexGenerator(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        for context_index in tqdm(context_indices, "Finding context pair", leave=False):
            ...
            if valid_indices:
                ...
                pick_start = context_left
                pick_end = context_right + 1
                if self.cfg.extrapolate_distance > 0:
                    num_views = extrinsics.shape[0]
                    pick_start = max(0, pick_start - self.cfg.extrapolate_distance)
                    pick_end = min(num_views, pick_end + self.cfg.extrapolate_distance)
    ...
    def save_index(self) -> None:
        self.cfg.output_path.parent.mkdir(exist_ok=True, parents=True)
        with (self.cfg.output_path).open("w") as f:
            json.dump(
