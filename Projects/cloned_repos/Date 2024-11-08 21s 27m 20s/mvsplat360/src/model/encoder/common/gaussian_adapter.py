...
class Gaussians:
    ...
    feature_harmonics: Float[Tensor, "*batch channels _"] | None
...
class GaussianAdapter(nn.Module):
    ...
    def __init__(self, cfg: GaussianAdapterCfg):
        ...
        self.n_feature_channels = cfg.n_feature_channels
        if cfg.n_feature_channels > 0:
            assert (
    ...
    def forward(
        ...
        if self.n_feature_channels > 0:
            scales, rotations, sh, feature_sh = raw_gaussians.split(
                (
                    3,
                    4,
                    3 * self.d_sh,
                    self.n_feature_channels * self.d_feature_sh,
                ),
                dim=-1,
            )
        else:
        ...
        if feature_sh is not None:
            feature_sh = rearrange(
                feature_sh, "... (c d_sh) -> ... c d_sh", c=self.n_feature_channels
            )
            feature_sh = (
                feature_sh.broadcast_to(
        ...
        return Gaussians(
            ...
            feature_harmonics=(
                None
                if feature_sh is None
                else rotate_sh(feature_sh, c2w_rotations[..., None, :, :])
            ),
            opacities=opacities,
    ...
    @property
    def d_in(self) -> int:
        return 7 + 3 * self.d_sh + self.n_feature_channels * self.d_feature_sh
